...
export const TEMPLATE_REWRITE_QUESTION = '【模板改写建议】按所选方案改写当前集完整正文，保留人物身份、动机、关键因果与剧本格式，不得省略未改部分；先对比再采用，不写回项目。仅在answer字段内返回JSON文本：{"kind":"template-rewrite","body":"完整改写正文（不超过9000字）","changes":["具体变化"]}。';
...
export function buildTemplateRewriteQuestion(plan: AdvisorTemplatePlan): string {
  return `【模板改写建议】按这个已选方案改写当前集完整正文：${JSON.stringify(plan)}。保留原人物身份、动机、世界观、关键因果和剧本格式，落实方案的结构节奏；不要仅给摘要或省略未改部分。不写回正式稿，等待用户对比采用。仅在answer字段内返回JSON文本：{"kind":"template-rewrite","body":"完整改写正文（不超过9000字）","changes":["具体变化"]}。`;
}
