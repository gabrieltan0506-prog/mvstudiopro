  ...
  async kind => {
    ...
    const click = async (label: string) => {
      const rect = await page.evaluate(label => {
        ...
        if(!button || button.disabled) throw Error('按钮缺失或不可用：'+label);
        const r=button.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};
      },label);
      await page.mouse.click(rect.x,rect.y);
    };
    await click(PREVIS_ACTION_LABELS.walk);
    expect(await page.$eval('[data-previs-action-window]',e=>e.textContent)).toContain('真实位移');
    expect(await page.evaluate(()=>Array.from(document.querySelectorAll('button')).find(b=>b.textContent==='添加所选动作')?.disabled)).toBe(true);
    await page.evaluate(()=>{const f=(window as any).fixture;f.setSpec({...f.spec,actors:f.spec.actors.map((a:any)=>({...a,end:[0,1.2],moveStartSec:0,moveEndSec:2}))});});
    await page.waitForFunction(()=>!Array.from(document.querySelectorAll('button')).find(b=>b.textContent==='添加所选动作')?.disabled);
    await click("播放动作预览");
