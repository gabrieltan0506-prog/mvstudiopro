...
(window as any).__toastHistory = () => toast.getHistory();
(window as any).__fetches=[];
import { resolveClipLocalSegmentIndex } from "@shared/manhuaScriptWorkbench";
import { manhuaFinalCutSourceKey } from "@shared/manhuaFinalCutSource";
import JSZip from "jszip";
import { defaultCanvasBlock } from "./canvasTypes";
/** 剪辑抽屉父级离线夹具：完整三段成片、真实剧本/素材/画布恢复。
...
function buildSeededCanvas() {
  ...
  // 本集三段均种入明确的离线已出片回执，真实完整性门禁仍由父组件执行。
...
if (localStorage.getItem("mv-manhua-writer-session-v1") === null) {
  ...
  laid.blocks.push(...other.blocks.map(b=>b.id.startsWith("clip-")?{...b,status:"done" as const,outputUrl:"https://example.com/other-episode.mp4"}:b));
  laid.edges.push(...other.edges);
  laid.blocks.push({...defaultCanvasBlock('video',0,0),id:'final-e01',status:'done',episodeIndex:1,outputUrl:'https://example.com/final.mp4',outputUrls:['https://example.com/final.mp4'],manhuaFinalVersions:[{url:'https://example.com/final.mp4',origin:'assemble',createdAt:1,sourceKey:(window as any).__stale?manhuaFinalCutSourceKey(queuedManhuaClipBlocks(laid.blocks,1,'seedance-2.5').map(b=>({blockId:b.id,episodeIndex:1,segmentIndex:resolveClipLocalSegmentIndex(b.id,b.prompt,1),clipUrl:String(b.outputUrl||''),trimInSec:b.manhuaEditTrim?.inSec,trimOutSec:b.manhuaEditTrim?.outSec,shotPieces:b.manhuaEditTrim?.shotPieces}))):undefined}]});
...
const deliveryProbe: { queue: any[]; packs: any[]; files: string[][]; alerts: string[] } = (window as any).__delivery = { queue: [], packs: [], files: [], alerts: [] };
window.alert = message => deliveryProbe.alerts.push(String(message));
const nativeObjectUrl = URL.createObjectURL.bind(URL);
URL.createObjectURL = blob => { if (blob instanceof Blob && blob.type.includes('zip')) { void JSZip.loadAsync(blob).then(async zip => {deliveryProbe.files.push(Object.keys(zip.files));deliveryProbe.packs.push(JSON.parse(await zip.file('manifest.json')!.async('string')));}); return 'blob:offline-zip'; } return nativeObjectUrl(blob); };
const nativeAnchorClick = HTMLAnchorElement.prototype.click;
HTMLAnchorElement.prototype.click = function() { if (!this.download) nativeAnchorClick.call(this); };
window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  const url = String(typeof input === "string" ? input : (input as Request).url ?? input);
  const method = (init?.method || "GET").toUpperCase();
  (window as any).__fetches.push({url,method});

  if (url.includes('mvAnalysis.queuePostProd')) { deliveryProbe.queue.push(JSON.parse(String(init?.body))); return new Response(JSON.stringify([{result:{data:{json:{jobId:'offline-extract'}}}}]),{headers:{'content-type':'application/json'}}); }
  if (url.includes('mvAnalysis.getPostProdJob')) return new Response(JSON.stringify([{result:{data:{json:{status:'succeeded',output:{url:'https://example.com/final.'+deliveryProbe.queue.at(-1)['0'].json.params.format,format:deliveryProbe.queue.at(-1)['0'].json.params.format}}}}}]),{headers:{'content-type':'application/json'}});
  if (/final\.(mp4|m4a|wav)/.test(url)) return new Response(new Uint8Array([1,2,3]));
  if (url.includes("op=manhuaAssembleCapabilities")) return new Response(JSON.stringify({ok:true,billingContractVersion:"manhua-assemble-v1",implicitMusic:false}),{headers:{"content-type":"application/json"}});
  ...
  if (method === "GET" && pollMatch) {
    if ((window as any).__assembleFailure) return new Response(JSON.stringify({status:"failed",error:"offline-render-failure"}),{headers:{"content-type":"application/json"}});
