...
export const GROWTH_CAMP_ANALYSIS_MODEL: GrowthCampModel = "moonshotai/kimi-k3";

export const GROWTH_CAMP_IMAGE_PIPELINE_DEBUG_NOTE =
  "每份素材上传完成即入队分析（可边传边析）· 语音 scan → 深度抽针 · 人设/趋势库注入 context";
...
function mapPollPercent(
  ...
  // queued 时不按等待时长虚涨进度，避免「排队中却显示 95%」
  const maxRatio = jobStatus === "queued" ? 0.28 : 0.96;
  const ratio = Math.min(maxRatio, elapsedMs / Math.max(maxWaitMs, 1));
  return Math.round(pollStart + ratio * (pollEnd - pollStart));
}

function growthJobStatusLabel(status: JobStatus): string {
...
export async function runGrowthCampVideoAnalysis(
  ...
  const job = await pollJobUntilTerminal(jobId, {
    ...
    onPoll: (tick) => {
      ...
      onGrowthPoll(tick);
      onProgressUpdate?.({
        percent: mapPollPercent(tick.elapsedMs, videoMaxWaitMs, progressRange, tick.status),
        phase: "video_analyze",
        label: tick.status === "queued"
...
export async function runGrowthCampAssetAnalysis(
  ...
  const emitPipelineProgress = (label: string, detail?: string) => {
    ...
    const collectPercent = (values: number[]) => {
      ...
      // 图片优先：图片已在分析时，整体进度跟图片轨，不被视频上传拖慢
      if (hasImages && analyzePollState.image && hasVideo && !analyzePollState.video) {
        return Math.round(analyzePollState.image.percent);
      }
  ...
  // 图片优先：传完立刻入队分析；视频上传与之并行，不阻塞图片分析
    ...
    hasVideo && video
      ? (async () => {
          try {
            ...
            await runVideoAnalyzeTask(uploaded);
          } catch (err: unknown) {
            taskErrors.push(err instanceof Error ? err.message : String(err));
          }
        })()
  ...
  if (videoUploadAndAnalyze) {
    analyzePromises.push(videoUploadAndAnalyze);
  }
