...
it("原稿变更显示stale并保留旧图，新原稿回执恢复ready", async () => {
 ...
 const built=await build({stdin:{resolveDir:process.cwd(),loader:"tsx",contents:`
  ...
  const refs=[{id:'a',role:'character',url:'https://offline.invalid/character.png',source:'upload',labelZh:'阿菁',claimedAnchorIds:['wa_char_aqing'],refDuty:'identity',primaryBindings:[{anchorId:'wa_char_aqing',duty:'identity'}]}];
  const canon={characters:[{id:'wa_char_aqing',role:'character',nameZh:'阿菁',lookZh:''}],locations:[],props:[]};
  ...
  function App(){const [blocks,setBlocks]=useState(initial);f.blocks=blocks;
   ...
   return <TooltipProvider><Workbench blocks={blocks} videoModel='seedance-2.5' topic='阿菁与墨菁' episodeCount={1} focusEpisode={1} onFocusEpisode={()=>{}} characterIds={[]} propIds={[]} outlineConfirmed={true} workflowPhase='storyboard' compactUi={false} customAssetRefs={refs} assetCanon={canon}/></TooltipProvider>;
  }
 ...
 try {
  ...
  writeFileSync(path.join(dir,'result.json'),JSON.stringify({before,stale,restored,errors},null,2));
 } finally {
