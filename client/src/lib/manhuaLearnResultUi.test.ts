...
describe("manhuaLearnResultUi soft-fail", () => {
  it("采样fps独立输入12或小数，不按319秒自动选档", () => {
    ...
    expect(restoreManhuaLearnVideoFps(undefined)).toBe(12);
    expect(restoreManhuaLearnVideoFps(null)).toBe(12);
  });
  ...
  it("采样fps按账号保存，旧设置损坏不改写有效12fps", () => {
    ...
    expect(readManhuaLearnVideoFps("user_b")).toBe(12);
  ...
  it("GCS快照续跑保留同剧319秒，不能串用其他剧设置", () => {
    ...
    expect(resolveManhuaLearnSnapshotVideoFps(continuation, "series_b", "https://douyin.com/video/b")).toBe(12);
  });
  ...
  it("服务端两部剧分别恢复319和默认300秒，并经篮子持久化保留", () => {
    ...
    expect(basket.find((item) => item.seriesKey === "series_1")?.continuation.nativeVideoFps).toBe(12);
