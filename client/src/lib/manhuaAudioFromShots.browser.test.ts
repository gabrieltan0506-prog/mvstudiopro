...
it('真实面板首次带入对白、保存恢复及手动清空不复活，全程不调用生成', async()=>{
 ...
 function App(){const [block,setBlock]=useState({...defaultCanvasBlock('video',0,0),id:'clip-e01-g01-seed',prompt:'目标时长：4秒',videoModel:'seedance-2.5'});const [revision,setRevision]=useState(0);f.block=block;f.addTake=()=>setBlock(b=>({...b,prompt:'目标时长：8秒',audioStudio:{...b.audioStudio,cues:b.audioStudio.cues.map(c=>{const q={...c,startSec:0,endSec:4};return {...q,takes:[{id:'fit-original',durationSec:4.944,gcsUri:'gs://test-bucket/original.wav',previewUrl:'https://test.invalid/original.wav',createdAt:'2026-09-20',inputKey:canvasAudioCueInputKey(q)}]};})}}));f.clear=()=>setBlock(b=>({...b,audioStudio:{...b.audioStudio,cues:[]}}));f.remount=()=>setRevision(n=>n+1);return <CanvasAudioStudioView key={revision} block={block} sourceShots={shots} services={services} onChange={audioStudio=>{f.updates++;setBlock(b=>({...b,audioStudio}));}}/>;}
