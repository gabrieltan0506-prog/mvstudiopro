...
import { Sparkles, Loader2, Download, Upload, Box, Image as ImageIcon, RotateCcw, Eye } from "lucide-react";
...
function ModelViewer({ glbUrl, thumbnailUrl }: { glbUrl: string; thumbnailUrl?: string | null }) {
  ...
  useEffect(() => {
    ...
    // Use model-viewer web component if available, otherwise show thumbnail
...
export default function VirtualIdol() {
  ...
  // Upload image via base64 → server tRPC
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    setUploading(true);
    try {
      // Convert to object URL for preview, then use it directly
      // The server will accept the image URL from the generated idol or uploaded URL
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="pt-32 text-center container">
          ...
          <p className="text-muted-foreground mb-8 max-w-lg mx-auto">AI 生成虚拟偶像形象，支持 2D 转 3D 模型</p>
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="pt-24 pb-16 container max-w-6xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">生成虚拟偶像形象，并将 2D 图片转换为 3D 模型</p>
        </div>
                      ...
                      <Card key={i} className="overflow-hidden bg-card/50 border-border/50 group">
                        <div className="relative aspect-square">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                            ...
                            <Button size="sm" variant="outline" className="bg-black/50 border-white/30 text-white" onClick={() => useImageFor3D(url)}>
                ...
                <CardContent className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        <div className="font-medium text-sm">Rapid 快速版</div>
                        <div className="text-xs text-muted-foreground mt-1">~30 秒 · {CREDIT_COSTS.idol3DRapid} Credits</div>
                        <div className="text-xs text-primary/70 mt-0.5">$0.225/次</div>
                      </button>
                      <button
                      ...
                      >
                        <div className="font-medium text-sm">Pro 高质量版</div>
                        <div className="text-xs text-muted-foreground mt-1">~60 秒 · {CREDIT_COSTS.idol3DPro} Credits</div>
                        <div className="text-xs text-primary/70 mt-0.5">$0.375/次</div>
                      </button>
                    </div>
                  </div>
