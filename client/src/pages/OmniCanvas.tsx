...
import { formatManhuaStageFrameSourceZh, listManhuaUsableStageFrames, toggleManhuaStageFrameAdoption } from "@shared/manhuaStageFrameAdoption";
...
import {
  ...
  setManhuaFinalPostProdReadError,
...
export default function OmniCanvas() {
  ...
  /** 段列头卡已落块过的 id；云恢复/换剧必须 clear，否则误判「用户删卡」清真源 */
  const materializedBoardIdsRef = useRef<Set<string>>(new Set());
  ...
  /** 段级导演板签名 url 过期：有 gcsUri 则现签刷新（与集级同口径，共用已签集合） */
  useEffect(() => {
    const stale: Array<{ ep: number; seg: number; gcsUri: string }> = [];
    for (const [epKey, segMap] of Object.entries(directorBoardBySegment)) {
      const ep = Number(epKey);
      if (!Number.isFinite(ep)) continue;
      for (const [segKey, e] of Object.entries(segMap || {})) {
        const seg = Number(segKey);
        const gcsUri = String(e?.gcsUri || "").trim();
        if (!Number.isFinite(seg) || !gcsUri) continue;
        if (resignedBoardGcsUriRef.current.has(gcsUri)) continue;
        stale.push({ ep, seg, gcsUri });
      }
    }
    if (!stale.length) return;
    let cancelled = false;
    void (async () => {
      ...
      if (cancelled || !resolved.length) return;
      setDirectorBoardBySegment((prev) => {
        const next = { ...prev };
        for (const r of resolved) {
          resignedBoardGcsUriRef.current.add(r.gcsUri);
          next[r.ep] = { ...(next[r.ep] || {}), [r.seg]: { gcsUri: r.gcsUri, url: r.url } };
        }
  ...
  const finalAssembleBlock = useMemo(() => {
  ...
  const finalVideoVersions = useMemo(
    () => ({
      ...
      urls: listManhuaFinalVideoVersions(finalAssembleBlock),
    }),
    [finalAssembleBlock],
  );
  ...
  const burnSubtitleRefreshDoneRef = useRef<Set<string>>(new Set());
  ...
  const generateManhua3dAsset = useCallback(
    async (assetRefId: string) => {
      ...
      if (currentModel3d?.status === "succeeded") {
  ...
  const syncBurnSubtitleBusy = useCallback(() => {
    setBurnSubtitleBusy(isManhuaSubtitleTaskBusy(burnSubtitleTaskGateRef.current));
  }, []);
  const burnSubtitleResultUrl =
    finalAssembleBlock?.manhuaFinalPostProd?.status === "succeeded"
      ? finalAssembleBlock.manhuaFinalPostProd.resultUrl || null
      : null;
  const burnSubtitleRecoveryError =
    finalAssembleBlock?.manhuaFinalPostProd?.errorZh ||
    normalizeManhuaFinalVersionIdentities(finalAssembleBlock?.manhuaFinalVersions).find(
      (version) => version.errorZh,
    )?.errorZh ||
    null;

  const updateFinalBlock = useCallback(
    (finalId: string, update: (block: CanvasBlock) => CanvasBlock) => {
      setBlocks((previous) => {
        let changed = false;
        const next = previous.map((block) => {
          if (block.id !== finalId) return block;
  ...
  const pollBurnSubtitleTask = useCallback(
    async (
      finalId: string,
      jobId: string,
      opts: { maxAttempts: number; notify: boolean; waitBeforeFirst?: boolean },
    ) => {
      const started = beginManhuaSubtitlePoll(burnSubtitleTaskGateRef.current, jobId);
      burnSubtitleTaskGateRef.current = started.state;
      if (!started.acquired) return;
      syncBurnSubtitleBusy();
      let lastError = "";
      try {
        for (let attempt = 0; attempt < opts.maxAttempts; attempt += 1) {
          if (attempt > 0 || opts.waitBeforeFirst) {
          ...
          try {
            ...
            if (job?.status === "succeeded") {
              ...
              const gcsUri = String(output.gcsUri || "").trim();
              if (!/^https:\/\//i.test(url) || !/^gs:\/\//i.test(gcsUri)) {
                throw new Error("烧字任务已完成，但长期成片身份缺失，请在后期任务列表取件");
              }
              burnSubtitleRefreshDoneRef.current.add(jobId);
              const before = blocksRef.current.find((block) => block.id === finalId);
              const preview = before
                ? applyManhuaFinalSubtitleBurnSuccess(before, {
                    jobId,
                    resultUrl: url,
                    resultGcsUri: gcsUri,
                  })
                : null;
              const accepted = Boolean(before && preview && preview !== before);
              const promoted = accepted && preview?.outputUrl === url;
              updateFinalBlock(finalId, (block) =>
                applyManhuaFinalSubtitleBurnSuccess(block, {
                  jobId,
                  resultUrl: url,
                  resultGcsUri: gcsUri,
                }),
              ...
              if (opts.notify) {
                if (!accepted) {
                  toast.message("烧字任务已完成，但原成片已被替换", {
                    description: "结果仍保留在后期任务记录，可从任务列表取件。",
                  });
                } else if (promoted) {
                  toast.success("烧字完成，新版已写回本集成片（原片仍可切回）");
                } else {
                  toast.success("烧字完成，已存入版本历史（当前选择未改变）");
                }
              }
              return;
            }
            if (job?.status === "failed") {
              const message = toManhuaSubtitlePublicError(job.error, "job");
              updateFinalBlock(finalId, (block) =>
                updateManhuaFinalSubtitleBurnStatus(block, {
                  jobId,
                  status: "failed",
                  errorZh: message,
                }),
              );
              if (opts.notify) toast.error(message);
              return;
            }
            updateFinalBlock(finalId, (block) =>
              updateManhuaFinalSubtitleBurnStatus(block, { jobId, status: "running" }),
            );
          } catch (error) {
            lastError = toManhuaSubtitlePublicError(error, "poll");
            if (attempt + 1 >= opts.maxAttempts) break;
          }
        }
        const message =
          lastError || "烧字仍在处理中；已保留任务，可刷新后继续取件";
        updateFinalBlock(finalId, (block) =>
          setManhuaFinalPostProdReadError(block, { jobId, errorZh: message }),
        );
        if (opts.notify) toast.message(message);
      } finally {
        burnSubtitleTaskGateRef.current = finishManhuaSubtitlePoll(
          burnSubtitleTaskGateRef.current,
          jobId,
        );
        syncBurnSubtitleBusy();
      }
    },
    [syncBurnSubtitleBusy, trpcUtils, updateFinalBlock],
  );

  const handleBurnSubtitle = useCallback(
    async (subtitleSrt: string) => {
      const started = beginManhuaSubtitleSubmit(burnSubtitleTaskGateRef.current);
      burnSubtitleTaskGateRef.current = started.state;
      if (!started.acquired) {
        toast.message("已有字幕烧录任务正在提交，请勿重复点击");
        return;
      }
      syncBurnSubtitleBusy();
      ...
      if (!videoUri) {
        ...
        burnSubtitleTaskGateRef.current = finishManhuaSubtitleSubmit(
          burnSubtitleTaskGateRef.current,
        );
        syncBurnSubtitleBusy();
        return;
      ...
      try {
        ...
        updateFinalBlock(finalId, (block) =>
          beginManhuaFinalSubtitleBurn(block, {
            jobId,
            sourceUrl: videoUri,
            sourceGcsUri,
          }),
        );
        await pollBurnSubtitleTask(finalId, jobId, {
          maxAttempts: 120,
          notify: true,
          waitBeforeFirst: true,
        });
      } catch (error) {
        toast.error(toManhuaSubtitlePublicError(error, "submit"));
      } finally {
        burnSubtitleTaskGateRef.current = finishManhuaSubtitleSubmit(
          burnSubtitleTaskGateRef.current,
        );
    ...
    [
      ...
      pollBurnSubtitleTask,
      queueBurnSubtitleMutation,
      syncBurnSubtitleBusy,
      updateFinalBlock,
      writerFocusEpisode,
    ],
  ...
  useEffect(() => {
    if (!user?.id) return;
    const binding = finalAssembleBlock?.manhuaFinalPostProd;
    if (!finalAssembleBlock || !binding?.jobId) return;
    if (binding.status === "queued" || binding.status === "running") {
      void pollBurnSubtitleTask(finalAssembleBlock.id, binding.jobId, {
        maxAttempts: 120,
        notify: false,
      });
      return;
    ...
    if (
      ...
      binding.resultGcsUri &&
      !burnSubtitleRefreshDoneRef.current.has(binding.jobId)
    ) {
      burnSubtitleRefreshDoneRef.current.add(binding.jobId);
      void pollBurnSubtitleTask(finalAssembleBlock.id, binding.jobId, {
        maxAttempts: 1,
        notify: false,
      });
    ...
    finalAssembleBlock?.id,
    finalAssembleBlock?.manhuaFinalPostProd?.jobId,
    finalAssembleBlock?.manhuaFinalPostProd?.resultGcsUri,
    finalAssembleBlock?.manhuaFinalPostProd?.status,
    pollBurnSubtitleTask,
    user?.id,
  ]);

  useEffect(() => {
    if (!user?.id || !finalAssembleBlock) return;
    for (const version of normalizeManhuaFinalVersionIdentities(
      finalAssembleBlock.manhuaFinalVersions,
    )) {
      if (
        version.origin !== "burn_subtitle" ||
        !version.jobId ||
        !version.gcsUri ||
        burnSubtitleRefreshDoneRef.current.has(version.jobId)
      ) {
        continue;
      }
      burnSubtitleRefreshDoneRef.current.add(version.jobId);
      void trpcUtils.mvAnalysis.getPostProdJob
        .fetch({ jobId: version.jobId })
        .then((job) => {
          const output = (job?.output ?? {}) as { url?: unknown; gcsUri?: unknown };
          const resultUrl = String(output.url || "").trim();
          const resultGcsUri = String(output.gcsUri || "").trim();
          if (
            job?.status !== "succeeded" ||
            !/^https:\/\//i.test(resultUrl) ||
            resultGcsUri !== version.gcsUri
          ) {
            throw new Error("历史成片链接刷新失败，长期版本身份仍已保留");
          }
          updateFinalBlock(finalAssembleBlock.id, (block) =>
            refreshManhuaFinalVersionIdentity(block, {
              jobId: version.jobId!,
              resultUrl,
              resultGcsUri,
            }),
        ...
        .catch((error) => {
          const errorZh = toManhuaSubtitlePublicError(error, "refresh");
          updateFinalBlock(finalAssembleBlock.id, (block) =>
            setManhuaFinalVersionReadError(block, {
              jobId: version.jobId!,
              errorZh,
            }),
          );
  ...
  const applyCloudDraftToUi = useCallback((draft: ManhuaCloudDraftPayload) => {
    ...
    setImmersiveWorkspaceView(workspaceViewForRestoredManhuaPhase(restoredPhase, Boolean(session.writerConfirmed)));
  ...
  /**
   * 用户资产原数组继续用于画廊、编辑与草稿；生成链只读与当前 canon 对齐的安全视图。
   * 这样旧剧角色图和旧 primaryBindings 都能保留，但不会静默进入新剧本提示词。
   */
  const consumableCustomAssetRefs = useMemo(
    ...
    [customAssetRefs, projectBible?.assetCanon],
  );
  ...
  /** 展示名映射：公开卡 nameZh 本身即匿名（赛道·爆款节奏 公开码），文案由服务端随卡下发 */
  const viralTemplateAnonLabelById = useMemo(() => {
    ...
    for (const group of manhuaViralTemplatesQuery.data?.groups || []) {
      (group.items || []).forEach((tpl) => {
        out.set(tpl.publicId, tpl.nameZh);
      });
    }
    return out;
  }, [manhuaViralTemplatesQuery.data?.groups]);
  ...
  const runDeps = useMemo<CanvasRunDeps>(
    () => ({
      ...
      manhuaAdoptedStageFrames: (block: CanvasBlock) => {
        ...
        const segmentIndex = Number(block.id.match(/-g(\d+)$/)?.[1]) || undefined;
        ...
        const shotIds = (block.previsStudio?.spec.actors ? [1, 2, 3] : [1, 2, 3]).map((i) => manhuaActionPlanShotId(episode, segmentIndex, i));
        const out: Array<{ refId: string; url: string; labelZh: string; sourceZh: string }> = [];
        for (const shotId of shotIds) {
          for (const ref of listManhuaUsableStageFrames(customAssetRefs, {
            shotId,
            episode,
            segmentIndex,
            actorIds,
            ...
            if (out.some((x) => x.refId === ref.id)) continue;
            out.push({ refId: ref.id, url: ref.url, labelZh: ref.labelZh || ref.id, sourceZh: formatManhuaStageFrameSourceZh(ref) });
          }
        }
        return out;
      },
  ...
  /**
   * 按**这一次操作本身**派生准备与试片口径，不拿全局试片状态一刀切。
   *
   * 原片编辑／延长走的是已有原片，本来就不是新试片：
   * 若当前集的试片还没批准，旧写法会给编辑预览也带上 pilotRun，
   * 被执行器的「10 秒试片不能代替原片编辑或延长」直接拒掉，
   * 于是编辑这一段**永远取不到确认**（0914 审查 P2）。
   */
  const deriveClipOperationOptions = useCallback(
    (block: CanvasBlock | undefined) => {
      const workMode = String(
        (block as Record<string, unknown> | undefined)?.seedance25WorkMode ?? "",
      ).trim();
  ...
  /** 段级导演板写入/删除（entry=null 删除该段；段图删完自动收掉集条目） */
  const setDirectorBoardMainForSegment = useCallback(
    (episodeIndex: number, segIndex: number, entry: { gcsUri: string; url?: string } | null) => {
      const ep = Math.max(1, Math.floor(episodeIndex) || 1);
      const seg = Math.max(1, Math.floor(segIndex) || 1);
      setDirectorBoardBySegment((prev) => {
        const next = { ...prev };
        const segMap = { ...(next[ep] || {}) };
        if (!entry || (!entry.gcsUri && !entry.url)) delete segMap[seg];
        else {
          segMap[seg] = {
            gcsUri: String(entry.gcsUri || "").trim(),
            ...(entry.url ? { url: String(entry.url).trim() } : {}),
          };
          if (segMap[seg]!.gcsUri) resignedBoardGcsUriRef.current.delete(segMap[seg]!.gcsUri);
        }
        if (Object.keys(segMap).length) next[ep] = segMap;
        else delete next[ep];
        saveManhuaDirectorBoardBySegment(next);
        return next;
      });
  ...
  const ingestDirectorBoardFile = useCallback(
    async (episodeIndex: number, file: File, segIndex?: number | null) => {
      const ep = Math.max(1, Math.floor(episodeIndex) || 1);
      const { uploadOneCanvasAsset } = await import("@/lib/canvasUpload");
      const asset = await uploadOneCanvasAsset({
        file,
      ...
      const seg = segIndex != null && Number.isFinite(segIndex) ? Math.max(1, Math.floor(segIndex)) : null;
      if (seg) {
        setDirectorBoardMainForSegment(ep, seg, { gcsUri, url: mainUrl });
        toast.success(`第${ep}集·段${String(seg).padStart(2, "0")}导演板已裁切并接入本段垫图`);
      } else {
    ...
    [
      cropDirectorBoardMutation,
      getSignedUrlMutation,
      setDirectorBoardMainForEpisode,
      setDirectorBoardMainForSegment,
    ],
  );
  ...
  const importWriterFromTextRef = useRef<((raw: string) => Promise<boolean>) | null>(null);
  ...
  const importAssetZipFile = useCallback(
    async (file: File) => {
      ...
      try {
         ...
         * 剧本必须**先于**资产写入。
         *
         * 同剧导入会保留既有参考；换剧仍须先完成备份确认并清旧资产。
         * 因此剧本仍要先落定，确认取消或解析失败时不得把新 ZIP 资产挂进旧剧。
         */
        ...
        if (bestScript) {
          ...
          if (ok) {
            const imported = await importWriterFromTextRef.current?.(bestScript.text);
            if (!imported) {
              toast.message("剧本未导入，已停止挂载这份资产包", {
                description: "旧剧本与旧资产保持不变。",
              });
              return;
            }
            scriptImported = 1;
        ...
        // 换剧备份里的 state.json 是完整快照：先恢复长期 GCS 身份与手调轨迹；
        // 同包若另带导演板图片，下面的裁切结果再按集覆盖主板。
        if (result.directorBoardState) {
          setDirectorBoardMainByEpisode(result.directorBoardState.mainByEpisode);
          saveManhuaDirectorBoardMainByEpisode(result.directorBoardState.mainByEpisode);
          setDirectorBoardBySegment(result.directorBoardState.bySegment);
          saveManhuaDirectorBoardBySegment(result.directorBoardState.bySegment);
          setDirectorBoardMotionOverlayBySegment(
            result.directorBoardState.motionOverlayBySegment,
          );
          saveManhuaDirectorBoardOverlayBySegment(
            result.directorBoardState.motionOverlayBySegment,
          );
  ...
  const expandWriterRoom = useCallback(async (opts?: { fromEpisodeOverride?: number }) => {
    ...
    try {
      ...
      const nextCustomAssetRefs = assetReview.refs;
      ...
      const templateHint = res.appliedTemplate?.publicId || res.appliedTemplate?.nameZh
        ? ` · 已应用剧情增强「${res.appliedTemplate.nameZh || viralTemplateAnonLabelById.get(res.appliedTemplate.publicId) || "剧情增强方案"}」`
        : "";
      toast.success(
      ...
      if (assetReview.markedRefCount > 0) {
        toast.message(`${assetReview.markedRefCount} 张旧设定图需要重新确认`, {
          description: "剧本中的对应人物、场景或道具描述已变化；图片仍保留，但确认前不会进入出片。",
        });
      }
    } catch (e: unknown) {
  ...
  const importWriterRoomFromText = useCallback(
    async (raw: string) => {
      ...
      const transition = classifyManhuaScriptImportTransition({
        currentSeriesTitle: writerPack?.seriesTitle,
        incomingSeriesTitle: res.pack.seriesTitle,
        hasExistingProject: risk.needsBackup,
      });
      const switchesSeries = transition === "new_series";
      const previousCanon =
        projectBible?.assetCanon ||
        (writerPack
          ? evaluateWriterPackAssetAndDensity({
      ...
      const assetReview = switchesSeries
        ? { refs: [] as typeof customAssetRefs, changedAnchorIds: [], markedRefCount: 0 }
        : markManhuaCustomAssetRefsForCanonChanges({
            refs: customAssetRefs,
            previousCanon,
            nextCanon: incomingCanon,
          });
      if (switchesSeries) {
        const allowed = await confirmManhuaSeriesSwitchWithBackup({
          risk,
          download: () =>
            downloadManhuaSeriesSwitchBackup({
              ...
              topic: factoryTopic.trim() || undefined,
              previousSeriesTitle: writerPack?.seriesTitle || undefined,
              incomingSeriesTitle: res.pack.seriesTitle || undefined,
              blocks,
              customAssetRefs,
              directorBoardMainByEpisode,
              directorBoardBySegment,
      ...
      let cleaned = stripManhuaFactoryCanvasArtifacts(blocks, edges);
      if (switchesSeries && risk.needsBackup) {
        const seriesCleared = stripManhuaSeriesAssetsForNewProject(
          cleaned.blocks,
          cleaned.edges,
        );
      ...
      setProjectBible(null);
      setCustomAssetRefs(assetReview.refs);
      ...
      toast.success(
        switchesSeries
          ...
          : `已导入 ${res.pack.episodes.length} 集《${res.pack.seriesTitle}》；既有图片、编辑版本与主图选择已保留，确认后按新人物表重新认领`,
      );
      if (assetReview.markedRefCount > 0) {
        toast.message(`${assetReview.markedRefCount} 张旧设定图需要重新确认`, {
          description: "对应剧本设定已变化；图片和主图选择仍保留，确认前不会用于出片。",
        });
      }
      return true;
    },
  ...
  const confirmWriterToDirector = useCallback((): boolean => {
    ...
    const spawned = spawnManhuaDramaStudio({
      ...
      wardrobePropContinuityIds: hardCast.wardrobePropContinuityIds,
      videoReverseOutputMode: factoryReverseMode,
      customRefs: consumableManhuaCustomAssetRefsForCanon(customAssetRefs, canon),
  ...
  const confirmWriterSeriesSpawn = useCallback(() => {
    ...
    const cleaned = stripManhuaFactoryCanvasArtifacts(blocks, edges);
    const nextBlocks = [...cleaned.blocks, ...spawned.blocks];
   ...
   * id 固定 board-eXX-sNN / board-eXX（版式按此归列；不带随机尾缀，天然幂等）。
   * 反向：头卡在画布上被删（且画布还有其它漫剧节点=非整体换稿）→ 视为删该板，
   * 清 store 真源，不复活——删除语义与卡面一致。
   */
  useEffect(() => {
    if (canvasMode !== "manhua") return;
    const expected = new Map<string, { url: string; ep: number; seg: number | null }>();
    for (const [epKey, segMap] of Object.entries(directorBoardUrlByEpisodeSegment)) {
      const ep = Number(epKey);
      if (!Number.isFinite(ep) || ep < 1) continue;
      for (const [segKey, url] of Object.entries(segMap || {})) {
        const seg = Number(segKey);
        const u = String(url || "").trim();
        if (!Number.isFinite(seg) || seg < 1 || !u) continue;
        expected.set(
          `board-e${String(ep).padStart(2, "0")}-s${String(seg).padStart(2, "0")}`,
          { url: u, ep, seg },
        );
      }
    }
    for (const [epKey, url] of Object.entries(directorBoardUrlByEpisode)) {
      const ep = Number(epKey);
      const u = String(url || "").trim();
      if (!Number.isFinite(ep) || ep < 1 || !u) continue;
      expected.set(`board-e${String(ep).padStart(2, "0")}`, { url: u, ep, seg: null });
    }
    // 真源已不含的 id 从「已落块」记忆剔除（换剧/删除后的残留会让下次同 id 上传被秒删）
    for (const id of Array.from(materializedBoardIdsRef.current)) {
      if (!expected.has(id)) materializedBoardIdsRef.current.delete(id);
    }
    const presentBoards = blocks.filter((b) => b.id.startsWith("board-e"));
    const presentById = new Map(presentBoards.map((b) => [b.id, b]));
    const hasOtherManhuaNodes = blocks.some(
      (b) => b.id.startsWith("keyart-") || b.id.startsWith("clip-") || b.id.startsWith("charsheet-"),
    );
    const toAdd: CanvasBlock[] = [];
    const toRefresh = new Map<string, string>();
    for (const [id, e] of Array.from(expected.entries())) {
      const hit = presentById.get(id);
      if (hit) {
        materializedBoardIdsRef.current.add(id);
        if (String(hit.outputUrl || "").trim() !== e.url) toRefresh.set(id, e.url);
        continue;
      }
      if (materializedBoardIdsRef.current.has(id) && hasOtherManhuaNodes) {
        // 画布上被删过 → 清真源（下轮 expected 不再含它）
        materializedBoardIdsRef.current.delete(id);
        if (e.seg != null) setDirectorBoardMainForSegment(e.ep, e.seg, null);
        else setDirectorBoardMainForEpisode(e.ep, null);
        continue;
      }
      const nb = defaultCanvasBlock("image", 0, 0);
      toAdd.push({
        ...nb,
        id,
        prompt:
          e.seg != null
            ? `第${String(e.ep).padStart(2, "0")}集·段${String(e.seg).padStart(2, "0")} 导演板 · 本段成片垫图`
            : `第${String(e.ep).padStart(2, "0")}集 导演板 · 全集共用垫图（无段级板时兜底）`,
        aspectRatio: "16:9",
        status: "done",
        outputUrl: e.url,
        outputUrls: [e.url],
    ...
    // 真源已删/换剧 → 收掉画布残卡
    const toDrop = presentBoards.filter((b) => !expected.has(b.id)).map((b) => b.id);
    for (const id of toDrop) materializedBoardIdsRef.current.delete(id);
    if (!toAdd.length && !toRefresh.size && !toDrop.length) return;
    setBlocks((prev) => {
      let next = prev;
      if (toDrop.length) next = next.filter((b) => !toDrop.includes(b.id));
      if (toRefresh.size) {
        next = next.map((b) => {
          const u = toRefresh.get(b.id);
          return u ? { ...b, outputUrl: u, outputUrls: [u] } : b;
        });
      }
      if (toAdd.length) {
        // 新头卡按段列归位（只在真的新增时排；URL 刷新/删除不动用户摆位）
        next = layoutManhuaEpisodeReadableChain([...next, ...toAdd], writerFocusEpisode, {
          assetCanon: projectBible?.assetCanon,
          characterSheetUrlById: collectManhuaCharacterSheetUrlById(next, projectBible?.assetCanon),
          customRefs: customAssetRefs,
        });
      }
      setEdges((eds) => {
  ...
  const handleRetakeClip = useCallback(
    (clipBlockId: string, variable: ManhuaRetakeVariable) => {
      ...
      const preparedBlock: CanvasBlock = {
        ...retakeBase,
        videoModel: retakeVideoModel,
        prompt: patchPromptForRetakeVariable(retakeBase.prompt, variable, attempt),
        status: "idle",
        error: undefined,
        manhuaClipQuality: undefined,
        outputUrl: undefined,
  ...
  const handleSelectFinalVideoVersion = useCallback(
    (url: string) => {
      const finalId = `final-e${String(writerFocusEpisode).padStart(2, "0")}`;
      updateFinalBlock(finalId, (block) => selectManhuaFinalVideoVersion(block, url));
      toast.message("已切换整集成片版本", {
        description: "预览与下载现在使用这一版；原版本仍保留。",
      });
    },
    [updateFinalBlock, writerFocusEpisode],
  );
            ...
            !(immersiveWorkbench && immersiveExtrasOpen) ? (
              ...
              >
                <ManhuaScriptWorkbench
                  ...
                  onSelectFinalVideoVersion={handleSelectFinalVideoVersion}
                  ...
                  onOpenWriterEditor={() => {
                    setImmersiveWorkspaceView("topic");
                    window.setTimeout(() => {
                      const editor = document.getElementById("manhua-writer-import-editor");
