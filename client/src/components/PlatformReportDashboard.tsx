 ...
 * 指標為決策輔助／模擬用；用戶面向文案避免裸露英文縮寫（CTR / MAB / UCB 等）。
 */
      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-12 flex flex-col gap-5 lg:col-span-5">
          ...
          <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {data.coreInsights.map((ins) => (
              <article key={ins.id} className="rounded-lg border border-gray-800/50 bg-[#131B2B] p-3.5">
                <h3 className="mb-1.5 text-sm font-semibold text-gray-300">{ins.title}</h3>
          ...
          <section className="rounded-lg border border-gray-800/50 bg-[#131B2B] p-3.5">
            <h2 className="mb-2 text-sm font-semibold text-gray-200">選題結構實例</h2>
            <div className="space-y-2">
              {data.topicStructureExamples.map((ex) => (
                <div
                  key={ex.title}
                  className="rounded-lg border border-gray-800/40 bg-[#0B0F19]/80 p-2.5 text-left"
                >
                  <div className="text-sm font-semibold text-white">{ex.title}</div>
                  <p className="mt-0.5 text-[11px] leading-snug text-gray-500">{ex.structure}</p>
        ...
        <div className="col-span-12 flex flex-col gap-5 lg:col-span-7">
          <section className="rounded-lg border border-gray-800/50 bg-[#131B2B] p-5">
            <h2 className="mb-4 text-lg font-semibold">平台雷達圖譜</h2>
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 md:items-stretch">
              <div className="min-h-[17rem] rounded-md border border-gray-800/40 bg-[#0B0F19]/60 p-3 md:min-h-[19rem]">
                <ResponsiveContainer width="100%" height="100%" minHeight={260}>
                  <RadarChart cx="50%" cy="50%" outerRadius="78%" data={subRadar}>
                    <PolarGrid stroke="#374151" />
              ...
              <div className="flex flex-col justify-center rounded-md border border-gray-800/40 bg-[#0B0F19]/60 p-4 text-xs leading-relaxed text-gray-400">
                <p className="text-sm font-semibold text-gray-200">熱榜 + 品牌契合</p>
                <p className="mt-3">
          ...
          <section className="rounded-lg border border-gray-800/50 bg-[#131B2B] p-5">
            <h2 className="mb-3 text-lg font-semibold">戰略升級方向 · 多版本對照與執行節奏</h2>
            <p className="mb-4 text-[11px] leading-relaxed text-gray-500">
              以動態測試思路對照不同敘事主線，「利用」偏向加量驗證成熟句型，「探索」保留新組合試錯空間。
            </p>
              ...
              {data.executionSuggestions.mabVariants.map((variant, idx) => (
                ...
                >
                  ...
                  <div className="mb-2 mt-3 text-center">
                    <span className="mr-2 text-sm font-bold text-gray-500">版本 {idx + 1}</span>
          ...
          <section className="rounded-lg border border-gray-800/50 bg-[#131B2B] p-4">
            ...
            <div className="space-y-2">
              <div className="flex border-b border-gray-800 pb-1.5 text-[11px] text-gray-500">
              ...
              {data.executionSuggestions.personalization.map((item) => (
                <div key={item.topicDirection} className="flex items-center py-1 text-sm">
                  <div className="w-1/2 truncate pr-2 font-medium text-gray-300" title={item.topicDirection}>
                    {item.topicDirection}
                  </div>
                  <div className="flex w-1/4 justify-center">
                    <div className="h-1.5 w-20 overflow-hidden rounded-full bg-gray-800 sm:w-24">
                      <div
                        className={`h-full ${
                  ...
                  <div className="w-1/4 text-right font-mono text-[13px] text-gray-400">
