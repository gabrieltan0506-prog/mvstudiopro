...
export default function ManhuaScriptWorkbench({
  ...
  const reviewClipTimelineSegments = reviewTimeline.tracks.find((track) => track.kind === "v2_clip")?.segments || [];
  const reviewMediaShotCount = reviewClipTimelineSegments.filter((segment) => segment.hasMedia).length;
  ...
  /**
   * 终审两条进度线必须读真实证据，不能把前两步写死成绿色勾。
   * 旧长片存在但当前段素材不齐／已失效时，也不能继续显示“初版生成完成”。
   */
  const finalContentReady = finalReviewChecklist.items.find((item) => item.id === "content")?.state === "pass";
  const finalDraftReady = finalContentReady && Boolean(finalVideoUrl) && !finalCutStale;
  const finalQualityReady = finalDraftReady && finalReviewChecklist.items
    .filter((item) => item.id !== "cut_fresh")
    .every((item) => item.state === "pass" || item.state === "not_required");
  const finalProgressStates = [
    finalContentReady,
    finalDraftReady,
    finalQualityReady,
  ...
  const finalCurrentProgressIndex = Math.max(0, finalProgressStates.findIndex((complete) => !complete));
  ...
  return (
    ...
    >
          ...
          <div key={phase.id} className="flex shrink-0 items-center gap-1">
            ...
            >
              ...
              <span className="min-w-0">
                ...
                {phase.gapZh ? (
                  <span className="mh-phase-gap block max-w-[150px] truncate whitespace-nowrap text-[9px] font-normal opacity-65">
                    {phase.gapZh}
                  </span>
                ) : null}
              </span>
      ...
      >
        ...
        {activePhase === "edit" || activePhase === "final" ? <button
          type="button"
      ...
      {activePhase === "final" ? (
        <div data-manhua-phase-panel="final" className="min-h-0 flex-1 overflow-y-auto p-3">
          <div className="grid min-w-0 gap-4 lg:grid-cols-3" data-manhua-final-dashboard>
            <section data-manhua-final-project-header className="flex items-center justify-between gap-6 rounded-xl border border-white/15 bg-white/[0.03] p-4 lg:col-span-3">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-16 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-emerald-500/20 to-violet-500/20 text-[10px] font-semibold text-white/55" aria-label="成片封面">
                  第{focusEpisode}集
                </div>
                <div className="min-w-0">
                  <h1 className="truncate text-lg font-bold text-white/95">{seriesTitle || topic || "未命名项目"} · 第{focusEpisode}集</h1>
                  <p className="mt-1 text-xs text-white/50">漫剧成片 · 约 {reviewTimeline.totalSec || totalSec}s · {episodeVideoLabelZh}</p>
                </div>
              </div>
              <div data-manhua-final-stage-line className="hidden min-w-[440px] items-start justify-between lg:flex">
              ...
              {finalSegmentIssues.length ? <section data-manhua-final-issues className="rounded-xl border border-amber-300/30 bg-amber-500/[0.06] p-4 lg:col-span-3">
                <h2 className="text-base font-semibold text-amber-100">存在 {finalSegmentIssues.length} 处需要处理的问题，才能通过终审</h2>
                <div className="mt-3 grid max-h-80 gap-2 overflow-y-auto">
                  {finalSegmentIssues.slice(0, 1).map(issue => <button type="button" key={issue.segment.index}
                  ...
                  {finalSegmentIssues.length > 1 ? (
                    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-2">
                      <div className="mb-2 text-xs text-white/55">其余 {finalSegmentIssues.length - 1} 项</div>
                      <div className="grid gap-2 sm:grid-cols-2">
                        {finalSegmentIssues.slice(1).map(issue => <button type="button" key={issue.segment.index}
                          data-manhua-review-issue={issue.segment.index}
                          className="flex min-h-11 w-full items-center justify-between gap-3 rounded border border-white/12 px-3 py-2 text-left text-sm hover:bg-white/10"
                          onClick={() => {
                            setActiveSegmentOverride(issue.segment.index);
                            const index = shots.findIndex(shot => shot.index === issue.segment.shots[0]?.index);
                            if (index >= 0) setShotIndex(index);
            ...
            <section data-manhua-final-section="quality" className="min-w-0 rounded-xl border border-white/15 bg-white/[0.03] p-3 lg:order-2"><div className="mb-2 flex items-center justify-between gap-2"><h2 className="text-sm font-semibold">质检结果</h2><strong className={finalReviewChecklist.failCount ? "text-rose-600" : "text-emerald-700"}>{finalReviewChecklist.items.filter(item => item.state === "pass").length}/{finalReviewChecklist.items.length} 项通过</strong></div>
        <div
        ...
        >
          ...
          <ul className="mt-3 space-y-2">
            {finalReviewChecklist.items.map((item) => (
              ...
              <button type="button" onClick={() => selectPhase("edit")} className="mt-3 min-h-11 rounded border border-white/20 px-3 text-xs">查看质检详情与修复</button>
            </section>
            ...
            <section data-manhua-final-section="timeline" className="min-w-0 rounded-xl border border-white/15 bg-white/[0.03] p-3 lg:order-1">
              ...
              <p data-manhua-final-media-shots={`${reviewMediaShotCount}/${reviewClipTimelineSegments.length}`} className="mt-1 text-xs text-white/55">{reviewClipTimelineSegments.length ? `${reviewMediaShotCount ? "当前裁切与排序" : "计划时间线"} · ${reviewTimeline.totalSec}s · ${reviewMediaShotCount}/${reviewClipTimelineSegments.length} 镜有成片` : "暂无剪辑计划 · 0镜"} · {finalCutStale ? "旧成片已失效，请重新合成" : finalCutVerified ? "当前成片来源已核对" : "最终成片尚未核验"}</p>
              <div data-manhua-final-timeline-steps className="mt-4 flex items-start justify-between">
      ...
      {activePhase === "outline" ? (
        ...
        >
          <div className="mx-auto w-full" data-manhua-outline-surface>
            ...
            <div data-manhua-series-summary-card className="relative mt-4 rounded-2xl border border-cyan-300/15 bg-gradient-to-br from-cyan-500/[0.08] to-white/[0.025] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.18)]">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      <div className={`flex flex-wrap gap-1.5 ${expanded ? "" : "hidden"}`}>
                        ...
                        {onGenerateCustomAssetFromLibrary ? (
                          <button
                            type="button"
                            data-manhua-action="generate-asset-from-library"
                      ...
                      >
                        ...
                        >
                          {group.kind === "flat" ? null : (
                            <div className="mb-2 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                              <span className="text-sm font-semibold text-white/85">{group.titleZh}</span>
                              <span
                              ...
                              <span className="text-xs text-white/45">· {group.refs.length} 张</span>
                            </div>
                          )}
                          {group.sharedNoteZh ? (
                            <p className="mh-hint text-[10px] leading-4 text-white/40">{group.sharedNoteZh}</p>
                          ) : null}
                          <div data-manhua-asset-card-media className={group.kind === "entity" && compactUi ? "grid grid-cols-[minmax(0,1fr)_3.5rem] gap-2" : ""}>
                          <div
                            className={
                              group.kind === "entity"
                                ? compactUi
                        ...
                        {group.refs.map((ref) => {
                          ...
                          {group.kind === "entity" && group.refs.length > 1 ? (
                            <div data-manhua-asset-version-rail className="flex max-h-60 flex-col gap-1 overflow-y-auto" aria-label={`${group.titleZh}版本缩略图`}>
                              {group.refs.map((version, versionIndex) => (
                                <button type="button" key={version.id}
                                  ...
                                  className={`w-full shrink-0 rounded border p-1 text-[10px] ${resolveManhuaAssetPreviewId(group.refs, assetPreviewByGroup[group.key]) === version.id ? "border-cyan-300 bg-cyan-400/10" : "border-white/15"}`}
                                  ...
                                  <img src={version.url} alt="" loading="lazy" className="mb-1 aspect-square w-full rounded object-cover" />
                                  版本 {versionIndex + 1}
      ...
      >
        {activePhase === "storyboard" ? (
          <header data-manhua-storyboard-header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-white/[0.03] px-4 py-3">
            <div className="min-w-0">
              <h1 className="text-base font-bold text-white/95">分镜工作区 · 第 {focusEpisode} 集</h1>
              <p className="mt-0.5 text-xs text-white/55">第 {activeShotNo || "—"} 镜 / {shots.length} 镜 · 左侧选镜，中央看图，右侧修改当前镜</p>
            </div>
            <span className="rounded-full border border-white/15 bg-white/[0.04] px-3 py-1.5 text-xs text-white/70">
        ...
        >
          ...
          {scriptTab === "shots" ? (
            <>
              ...
              <details data-manhua-storyboard-preparation className="mt-2 shrink-0 rounded-lg border border-white/10 px-2 py-1">
                <summary className="cursor-pointer text-[10px] font-semibold text-white/60">视觉简报与生成准备</summary>
              {episodeStillCount === 0 ? (
        ...
        <aside
          ...
          className={
            `max-md:!w-full max-md:min-w-0 ${narrowWorkbenchColumn !== "assets" ? "max-md:!hidden" : ""} ` + (storyboardThreeColumn
              ? "h-full min-h-0 min-w-0 overflow-y-auto border-r border-white/10 p-2"
              : immersive
