...
/** supervisor / admin 专用：漫剧工厂阶段日志 + 注入摘要（不对一般用户展示） */
  ...
  return (
    <div className="mb-4 rounded-2xl border border-[#2b1f52] bg-[#140b31] p-4">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        <div>
          <div className="text-[10px] uppercase tracking-[0.14em] text-white/35">阶段日志</div>
          {entries.length === 0 ? (
            ...
            <div className="mt-2 max-h-56 space-y-1 overflow-y-auto pr-1">
              {entries.map((line) => (
                <div
                  key={line.id}
                  className={`font-mono text-[11px] leading-5 ${levelClass(line.level)}`}
                >
                  ...
                  {line.detail ? <div className="pl-2 text-white/55">{line.detail}</div> : null}
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-xl border border-white/10 bg-black/35 p-3 font-mono text-[11px] leading-5 text-[#d7d0ef]">
