...
**WO 战略（补劣势抓机会）**：第一阶段不要自己拍，用图文 + 文本稿的形式发到小红书做内容验证，验证选题后再外包视频制作。
...
3. **「只盯着哔哩哔哩」**——视频号的 60+ 用户增速已达 118%，要尽快创建第二阵地
...
function exportSampleAsPdf(
  ...
  //   2) 在 wrapper 内撒大量真实 DOM 元素（logo + 对角文本）做绝对定位 —— 不依赖任何打印配置
  //      puppeteer 自动分页时，每个页位置都会自然包含撒在该位置的水印元素，每页 ≥ 4 个 logo + ≥ 12 个文本水印
  ...
  if (cloneBody) {
    ...
    // —— 文本水印矩阵（4 列 × N 行，更密更不抢戏）—— 半透明深咖啡文本
