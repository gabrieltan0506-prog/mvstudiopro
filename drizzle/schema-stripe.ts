...
export const stripeCustomers = pgTable("stripe_customers", {
  ...
  userId: integer("userId").notNull().unique(), // FK → users.id
...
export const creditBalances = pgTable("credit_balances", {
  ...
  lifetimeEarned: integer("lifetimeEarned").default(0).notNull(), // 累計獲得
  lifetimeSpent: integer("lifetimeSpent").default(0).notNull(), // 累計消耗
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
export const creditTransactions = pgTable("credit_transactions", {
  ...
  amount: integer("amount").notNull(), // 正數=充值, 負數=扣費
...
// 功能使用日誌
// ═══════════════════════════════════════════
export const stripeUsageLogs = pgTable("stripe_usage_logs", {
  id: serial().primaryKey(),
  userId: integer("userId").notNull(), // FK → users.id
  ...
  isFreeQuota: integer("isFreeQuota").default(0).notNull(), // 0 = paid, 1 = free quota
  description: text("description"),
...
export const coupons = pgTable("coupons", {
  ...
  discountPercent: integer("discountPercent").default(0).notNull(), // 百分比折扣 (10 = 10% off)
  discountAmount: decimal("discountAmount", { precision: 10, scale: 2 }), // 固定金額折扣
  maxUses: integer("maxUses").default(100).notNull(),
