...
## 2026-09-13 独立审查修正与发布准备

用户已授权提交推送开 PR、独立子代理审查、修复并复审，无问题且合并前确认无在途任务/冲突部署才合并。原始代码176e58eb与日志3261afa2已推；当前附加修复仍需同一代理复审。

独立审查发现3项P2并已实现修正：明确合成终态后可主动生成新版本，旧任务快照与成片保留；分镜5分钟截止后经原子终态恢复解析稿或明确失败，网络错误不冒充404，迟到旧执行不交付/扣费；视频出站仅保留该镜正文，不携带整首歌词/整张分镜。新增真实离线浏览器覆盖成功/失败后新合成、未知任务只查原号、分镜失败确认后的新轮，以及服务端中断/迟到执行竞态。

修正后31文件400项通过（39.83秒）；类型检查和非增量服务端构建退出0，Vite退出0（46.42秒），未做真实付费调用。此前新增测试有两处错误地等待未知请求转error状态而超时，已改为核对实际错误信息及原任务保留，未延长超时冒充通过；修正后的6项浏览器测试通过。原始结果保留在review-*.log。

运行初查正式d892541f602228的DB、9条画布视频、23条研究记录均无在途；另有0801696b2d6e38隔离预演机器运行Blender，不据此初查直接放行合并，须复审后紧邻动作复查所有相关进程与部署。


## 2026-09-13 外层输入恢复补充修正

独立复审发现未知分镜期间外层音乐提示词仍可编辑并使effect清除原请求。现以共享hasPendingMusicMvPlan同时锁定外层输入和同步effect；即使外部状态更新仍保留原请求与输入快照，恢复原稿后明确提示差异，用户主动准备新稿时归档原请求。正向从外层输入/云更新到原请求查询，反向从恢复稿到提交输入，均核对同一requestId；不新增付费调用。

3文件15项通过（5.68秒），pnpm check与非增量服务端构建退出0，Vite退出0（35.04秒，仍有既有大块警告）。原始日志为review-outer-edit.log、outer-check.log、outer-build.log、outer-vite.log。正式账号付费与线上恢复未实跑，仍为部分验证。
