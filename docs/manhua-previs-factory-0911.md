...
- **竖屏另有一次性前置扫描**（0912 条件收紧）：判定收紧是否会切到人，最坏跑 2×帧数次
  ...
  **主导项是换帧与场景求值**（`frame_set` + `view_layer.update()`），约占七成，而且**它本身
  ...
  砍骨骼数没用。**
  本机实测前置扫描净耗时（720 帧 / 9:16，三次取最小）：单角色 1.06 秒、三角色 1.14 秒
  （第二趟通常早退，满扫约 1.8 秒）、六角色 2.45 秒。三角色 30 秒落在原 17.8 秒基线上
  约 +6%，最坏（强制两趟满扫）约 +10%。六角色 30 秒本来就被 `PREVIS_RENDER_UNIT_BUDGET`
  事前拒绝，列在这里只作上界参考。
