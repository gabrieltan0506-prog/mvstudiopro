...
- **竖屏另有一次性前置扫描**（0912 条件收紧）：判定收紧是否会切到人，最坏要跑 2×帧数次
  `frame_set`，扫全部骨骼。本机实测三角色 30 秒 prepare 从 17.8 秒涨到 21.6 秒（+21%）；
  成本只随帧数涨、不随角色数线性涨，因此**不计入 2700 这个口径**（它建模的是渲染的
  帧数×角色数），而是吃构建预留的那部分。下次调预算的人要知道构建预留没有原来那么宽。
