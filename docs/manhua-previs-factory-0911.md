...
- **竖屏另有一次性前置扫描**（0912 条件收紧）：判定收紧是否会切到人，最坏跑 2×帧数次
  ...
  **主导项是换帧与场景求值**（`frame_set` + `view_layer.update()`），约占七成，而且**它本身
  ...
  砍骨骼数没用。**
  **前置扫描净耗时（强制两趟满扫的最坏口径，720 帧 / 9:16，插桩量扫描块本身，多次取最小）**：
  ...
  与上面的每帧模型自洽（近似随角色数线性）。**调预算只认这一组。**
  实际运行常比这短——收紧挤不下时第二趟会提前返回——但早退能省多少取决于站位，不可依赖。
  端到端墙钟在本机噪声极大（同一条命令能差五倍），不可采信，只认插桩测到的扫描净耗时。

  落到预算上：预算内最大的竖屏规格是三角色 30 秒（2160 单位），扫描约 2.5 秒，
  落在原 17.7 秒基线上约 **+14%**，对 600 秒生产时限是 0.4%，撞不到。
