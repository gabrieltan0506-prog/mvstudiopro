...
- **竖屏另有一次性前置扫描**（0912 条件收紧）：判定收紧是否会切到人，最坏跑 2×帧数次
  ...
  **主导项是换帧与场景求值**（`frame_set` + `view_layer.update()`），约占七成，而且**它本身
  随角色数涨**——插桩实测 1 / 3 / 6 角色分别约 0.35 / 0.92 / 1.91 毫秒每帧。骨骼采样约
  0.005 毫秒每点，是次要项。**所以要优化这段就去减少换帧次数（两趟合一、抽帧步进），
  砍骨骼数没用。**
  前置扫描净耗时（720 帧 / 9:16，插桩量的是扫描块本身，不是端到端墙钟——端到端在本机
  噪声极大，同一条命令能差五倍，不可采信）。**两次独立测量、口径不同，都列出来**：
  - 真实路径（第二趟挤不下时会早退）：单角色 1.06 秒、三角色 1.14 秒、六角色 2.45 秒。
  - 强制两趟满扫（最坏）：单角色 0.75 秒、三角色 2.48 秒、六角色 4.82 秒。
  单角色两个口径本该一样（它判 tight，两趟都跑满），实测 1.06 与 0.75 的差距就是本机负载噪声，
  别把这类数字当精确值用。
  **调预算只看最坏那一行**：预算内最大的竖屏规格是三角色 30 秒（2160 单位），扫描约 2.5 秒，
  落在原 17.8 秒基线上约 +14%，对 600 秒生产时限是 0.4%，撞不到。
  六角色 30 秒本来就被 `PREVIS_RENDER_UNIT_BUDGET` 事前拒绝，列在这里只作上界参考。
