...
- **竖屏另有一次性前置扫描**（0912 条件收紧）：判定收紧是否会切到人，最坏跑 2×帧数次
  `frame_set`，每帧扫全部角色的全部骨骼。
  成本 ≈ 帧数 ×（`frame_set` + 角色数 × 骨骼数 × 采样）；**角色数越多这项越接近线性**，
  不要按「只随帧数涨」去估。插桩实测净耗时（720 帧 / 9:16）：单角色 1.05 秒、三角色 2.13 秒；
  拟合出 `frame_set ≈ 0.35 ms/帧`、每骨骼采样 ≈ 0.023 ms，`frame_set` 占比从单角色的 48%
  掉到六角色的约 14%——主导项已经是骨骼采样，不是换帧。
  三角色 30 秒落在原 17.8 秒基线上约 +12%。这部分**不计入 2700 这个口径**（它建模的是
  渲染的帧数×角色数），而是吃构建预留的那部分。下次调预算的人要知道构建预留没有原来那么宽。
