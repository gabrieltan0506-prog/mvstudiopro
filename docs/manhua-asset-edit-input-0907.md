...
已实现但未线上验收：共享解析结果增加 isFallback；两个原数组 API 仍返回同样的 shots，选源顺序、正则、两行判据、180字占位种子、对白／机位覆盖完全保持。工作台四个统计入口共用真实解析标记，占位显示“占位规划·未解析原稿”；其余仅称“画布分镜规划”，因为模板本身也能解析成行，不等于用户原稿。时长明确称规划，不更改实际数字。

双向追链：当前集 blocks → 原 selectedText → 真实逐镜／段表解析 → 同一结果的 shots 与 isFallback → 工作台派生状态／四处标签；反向从标签回到同集同源解析，不从 writerPack 是否存在或镜数18猜测来源。媒体、持久化、API与扣费没有新增写入，生成／节点重跑继续只消费原数组接口。

验证原始结果：

```text
pnpm exec tsx -e …（git 74a60ed 原选源函数与当前函数）
8 组完整数组逐字段相等，镜数分别18/6/6/6/18/2/18/2，退出0

pnpm exec tsx -e …（git 74a60ed 原共享解析文件内存打包与当前解析函数）
8 组完整数组逐字段相等，镜数18/18/18/18/18/2/2/2，退出0

相关消费者回归：13文件202项通过；来源专项：3文件35项通过
pnpm check：退出0
pnpm exec vite build：3557模块，17.66秒，退出0（既有警告保留）
pnpm exec vitest run --silent：537文件通过/2失败/2跳过，5182项通过/2失败/4跳过
Duration 148.72s，退出1，仍为两个原有7201 CLI断言
```

既有风险被专项测试如实揭示：beats 只有两条对白覆盖行时，旧选源正则会抢过 reverse 成稿，再落入默认骨架；本次只标出回落，不擅自改变该选源行为。后续修正应独立验证输出来源和覆写契约。
