...
async function main() {
  ...
  for (const frame of frameList) {
    const url = await getSignedGcsReadUrl({ objectName: String(frame.objectName), expiresInSeconds: 6 * 24 * 3600 });
  ...
  const tags = Object.entries(cl)
    ...
    .map(([k, v]) => `<div style="margin:4px 0"><b style="color:#e8c66a">${esc(k)}</b>：${(v as unknown[]).map((t) => `<span style="background:#1d2733;border-radius:10px;padding:2px 10px;margin:2px;display:inline-block">${esc(t)}</span>`).join(" ")}</div>`)
    .join("");
  const summaryCards = (["beatStructureZh", "moodArcZh", "reusableZh", "genPromptHintZh"] as const)
    .map((key) => `<div style="background:#141b24;border-left:3px solid #e8c66a;padding:10px 14px;margin:8px 0"><b>${key}</b><br><span style="color:#9db4d0">${esc(card![key])}</span></div>`)
    .join("");
...
<p style="color:#5d6b80;font-size:.8em;margin-top:36px">帧图与本页为 GCS V4 签名链接（6 天）· raw/parsed/GLM 卡永久存 GCS · 本页由代码从模型 JSON 确定性渲染</p></div>`;
  ...
  const reportUrl = await getSignedGcsReadUrl({ objectName, expiresInSeconds: 6 * 24 * 3600 });
