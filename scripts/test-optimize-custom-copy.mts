/**
 * 本地烟测：自定义文案深度优化（会调用 Evolink GPT-5.5 或 Gemini 兜底，可能产生费用）。
...
if (!hasEvolink && !hasGemini) {
  console.error("SKIP: 需要 EVOLINK_API_KEY 或 GEMINI_API_KEY（.env.local）才能跑 live 烟测");
  process.exit(1);
}

console.log(`凭据: ${hasEvolink ? "Evolink" : "仅 Gemini 兜底"}`);
