...
2. **采样**：按剧集顺序每轮采 **8–10** 集；单集学完**立刻删视频**，只留 digest / 提案 JSON（GCS `series/<key>/episodes/`）
3. **分析门槛**：同一合集累计 **≥16** 集才出一张总分析提案（目标凑满约 **20**）
4. **网页即时可见**：分集摘要 + 进度 +（满门槛时）总分析预览出现在 Platform 本页；**看完再决定**是否点「批准进库」
5. **回退**：云端失败时自动复制本机命令；或「导出学习 JSON」后本机  
   `pnpm run manhua:template-learn -- --rising-json <文件> --rank N`
6. **抽帧密度**：前 5s 钩子；约每 10s；高潮窗约每 3s
7. 用户明文「批准进库」→ GCS `approved/` → 编剧室合并列表可选

依赖（云端）：Fly 镜像已装 yt-dlp/ffmpeg。本机回退需本机同款工具。

搜索页 URL 可能无法直接下片：优先用合集/成片页链接。合集可解析集数 &lt; 16 时需换合集或补链。
