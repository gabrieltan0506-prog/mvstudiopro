...
1. **首选**：Platform 单行「学节奏」→ 云端 Job `manhua_template_learn`（下片+语音 Flash+读帧 Terra）→ GCS 提案  
2. **回退**：云端失败时自动复制本机命令；或「导出学习 JSON」后本机  
   `pnpm run manhua:template-learn -- --rising-json <文件> --rank N`  
3. **抽帧密度**：前 5s 钩子；约每 10s；高潮窗约每 3s  
4. 用户明文「批准进库」→ GCS `approved/` → 编剧室合并列表可选  
