...
| 原生读片设置 | PR1328：原速319秒/12fps，Gemini MEDIUM、GLM整集与系列medium，无thinkingBudget。2ac2117的v24时间说明实验两发STOP、第二发过结构门但原帧错场/合镜，尚未达标。用户否决1.0（未运行），指定首发0.65并复用旧成功配置；候选v25三档0.65/0.65/0.6、最低0.6，后两档来自b948，保留v24 prompt/Schema及用户后续分支要求。须完成类型/回归/构建、同PR推送及隔离同源原帧验收。既有五片继续复用；先1片稳定后2、3片，每片最多三次，不逐片GLM、不生成假整集；失败继续查因调优，实测过关才冻结，暂不合并或正式部署。 |
