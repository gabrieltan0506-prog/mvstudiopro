...
| 原生读片设置 | PR1328：当前原速319秒/12fps，Gemini MEDIUM、GLM整集与系列medium，无thinkingBudget。7d492a4同源单片0.70/0.60/0.55三发均STOP、进程正常退出，第三发过结构门但三发原帧100/165/260均错场，未达到质量要求。第二发字幕419与原260秒同句，支持MM:SS误拼为整数的时间格式问题，但禁止自动改整份JSON。下一候选仅在生产prompt澄清局部钟表转秒及全片偏移，温度梯度/Schema/fps/门禁均不改，仍须推送同PR与隔离同源实测。既有五片继续复用；先1片稳定后2、3片，每片最多三次，不逐片GLM、不生成假整集；实测过关才冻结，暂不合并或正式部署。 |
