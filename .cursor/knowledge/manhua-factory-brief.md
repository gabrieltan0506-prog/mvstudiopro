...
| 原生读片设置 | PR1328：分片秒数与采样fps独立设置，旧记录默认300秒/10fps；当前原速319秒/12fps。Gemini MEDIUM、GLM整集与系列medium，不传thinkingBudget。9352588五片9次Gemini后一次整集GLM已返回，但质量未通过。用户改为一片稳定后两片、再三片，每片最多三次总尝试；0.65首发已查出实际秒位错配，下一轮按授权仅首发试0.70，保留重试0.60/0.55，其余prompt/Schema/参数不动。新增选片诊断复用生产请求/重试/解析，只使用既有GCS片，保留全片计划，不逐片GLM、不生成假整集；未知尝试费用不能被成功重试吞掉。最终候选尚待全仓复验、推送镜像和新付费实跑；实测过关后才冻结，倍速未实现。 |
