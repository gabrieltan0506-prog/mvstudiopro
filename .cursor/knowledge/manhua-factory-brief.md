...
| 原生读片设置 | 0901现行：自定义分片秒数和fps逐次任务生效；Gemini MEDIUM、无thinkingBudget，温度锁定0.7/0.65/0.6。只有门禁失败降档；503/429/资源耗尽等待60秒同温重跑，每个当前温度档最多资源重试3次。三档完成后由Qwen 3.8 Max必须三选一，无论schema是否通过都不取消选择；按结构、时间轴、画面、逐镜、字幕/重点时刻和声音的综合可用性判断，只有被选稿进整集GLM。所有分片先到终态才合成，尾片与普通片同规则，模型扇出并发上限5。面板按Debug Panel展示真实门禁、拒因、温度、资源重试与Qwen选择，不再用“结构完成”概括失败。 |
