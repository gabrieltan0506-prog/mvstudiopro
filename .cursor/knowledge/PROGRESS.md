...
- 当前用户明确要求 `reusableZh` / `genPromptHintZh` 不允许缺省：新读片 schema 必填、段级非空检查；整形 schema 同步非空约束。仅这两栏读片契约变更，已更新冻结摘要，其他模型、温度、prompt 与重试配置保持。
...
- 限制：当前模板字符串仍有原有600字符容量上限，本次未扩大字段容量；新schema会改变请求指纹，不能承诺重新点学习一律零费用，旧报告补栏应从已有证据导出。
