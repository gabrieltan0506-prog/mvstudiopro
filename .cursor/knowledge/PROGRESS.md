...
- 本地 `pnpm check`、`pnpm build`、21 个相关测试文件 195 项与 `git diff --check` 通过。全仓 4111 通过／7 跳过／6 失败，其中本刀旧箭头断言 1 项已修；余 5 项在 `origin/main` 同源存在（14400 秒新上限仍用 7201 旧断言 2 项、owner 文案常量化后仍查旧字面 3 项）。尚未 commit、push、开 PR、合并、部署或真实付费出片，不能称线上完成。
