...
**PR1328思考档位统一（2026-08-31，用户明确授权，待五片实测）**：Gemini保持MEDIUM；GLM整集结构化、系列聚合与两份旧整形探针统一引用共享medium常量，其他参数/路由/Schema不改。聚合快照哈希纳入reasoningEffort，旧max结果保留但不冒充medium的新验收。主备实际请求体和旧缓存隔离目标236项通过；本轮验收流程仅五片各一次Gemini→全部收齐后一次整集GLM，不额外跑系列聚合，不逐片调用GLM；正常无重试时共6次模型请求。实测通过前不新增冻结机制。
