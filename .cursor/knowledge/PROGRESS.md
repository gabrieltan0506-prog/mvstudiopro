...
## 2026-09-08 · 每句情绪中文勾选（本地部分验证）

用户后续授权审查后推送（未授权本增量合并/部署）。独立5文件46项通过、未发现P0–P2；补完整工厂切两段与自由画布节点的情绪勾选/保存/恢复/运行禁用2项浏览器回归通过，pnpm check再次退出0。生产权限及自由画布2.5多模态入口限制不改；共10个文件准备提交到当前功能分支。

按用户明确要求，共用音轨组件新增中文复选框菜单，19种既有控制分情绪/表达方式，可组合、反选和恢复自然表达，声线独立；高级旧标签折叠保留。真实组件请求为[serious][empathetic]别怕，站我身后。；选择不生成，改后取消旧采用但保留音频。33文件369项、类型、服务端/Vite构建及diff检查通过；本批未提交/推送/上线，未全仓或声音效果验收。完整双向追链、九层状态和文件见docs/canvas-dialogue-emotion-0908.md。
