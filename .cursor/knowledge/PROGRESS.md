...
## 2026-09-08 · 资产长开页过期显示（本地部分验证）

真实新增图片已成功，七天输出被工作台一次性续签替换为一小时，长开后破图；原任务HTTP200且2241106字节可取。初始化保留同对象有效签名，十三个共享图片消费点增加本机优先／同对象一次有界续签，只改DOM；放大/裁剪按独立资产ID取当前地址。生成、费用、退款、权限和持久化契约未改，不重复生成。6文件60项及独立4文件36项通过，类型/无增量类型/服务端及Vite构建退出0；全仓5210通过、4跳过、2个既有CLI7201断言失败。完整证据和双向追链见 `docs/manhua-asset-preview-refresh-0908.md`；尚未发布或验新版原页面。
