...
## 2026-09-08 · 知识卡全稿图文阅读与方案生成（部分验证）

本轮用户授权知识卡独立PR并在通过发布门禁后推送合并。入口统一为全部PDF物理页/图片/连续文字段阅读，模型给精简、均衡、完整真实方案；完整内容四页足够时仅完整四页。确认后生成冻结正文，原稿构图作为独立视觉参考实际传入生图，保留横版16:9、4K及左侧/居中选择、现有页价与手输长文50/30积分确认。原始与解析JSON永久分存，原件generation、完整来源覆盖、稳定页身份防重、低余额原账恢复和成品故障对账已加测试；旧素材及成品保留，当前版次导出不混旧图。专用单并发阅读worker不挤占原主队列领取槽。同源用户PDF本地276/276页渲染并解码；最终同步main f27e65f后55文件613项通过，无增量类型检查与Vite构建退出0，结果见 docs/2026Sep08/knowledge-card-full-reading-audit.md。未做线上付费阅读/生图质量与真实账单验收，用户将发布后重跑；不能据此称售卖成品质量已验证。
