...
用户现明确使用内置原画布，十五分钟自动检查已依令删除；生图、三段十秒视频及必要时另增三至五段、TTS/BGM/剪辑均有当前授权。实际页面六张参考均可解码，缺场景／道具图及三份剧本资产表。点击黑翼图编辑复现 `prompt() is not supported`，没有建单；新增页面内输入弹层，保留原成本确认、资产ID、续签、画幅和新图追加链。九文件130项回归通过，含四项真实离线浏览器；类型、无增量类型、格式和Vite通过。全仓5141通过、4跳过、2项既有CLI上限断言失败。没有借新授权重复购买常态模型，没有新图片或视频产物；新PR发布载体待用户明确，详见 docs/manhua-asset-edit-input-0907.md。
