...
**新加坡套餐直读 GCS 探针（真实模型调用已通过，生产尚未切换）**：Fly 内使用 `DASHSCOPE_SG_PLAN_KEY` 配新加坡 Token Plan 专属端点，通过 OpenAI Chat Completions `video_url` 直接读取 GCS V4 签名的 4 秒红→蓝视频；GCS 返回 206，模型返回 200 / stop，输入 372、输出 90 tokens，并准确识别颜色变化。把套餐密钥误配普通新加坡业务空间端点会 401 且 token 为 0。测试对象和探针文件已删除。项目决定后续取消 OSS 中转，改为 GCS 临时切片阅后即删，以取消 OSS 外网读取计费；当前生产执行器尚未完成该切换，不得把探针成功写成线上已通。
