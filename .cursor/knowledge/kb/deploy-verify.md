...
## PR1328 隔离调优例外范围（2026-08-31）

用户授权的调优仅在无业务services的隔离机2870151a097278执行，正式机d892541f602228不得跟随换版。build-only/push镜像后，以已推PR提交和1028源码证明验明身份，再运行同源生产CLI；凭证留Fly。前轮v24退出0但原帧质量失败，不等于可合并。当前首发0.65、旧版后两档0.65/0.6候选仍须实测；1.0候选取消未运行。每轮至多三次尝试，保存全部原稿/用量及失败原因，质量不合格时继续证据驱动调优，不以测试轮次结束代替用户要求达标。
