...
function fallbackEnglishPrompt(text, maxChars) {
...
function normalizeOutput(text, maxChars) {
 ...
 */
async function attemptTranslateOnce(src, maxChars, apiKey, baseUrl, model) {
  try {
    const r = await fetch(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Translate to concise English for image/video generation. Output one single-line prompt only, comma-separated phrases, no markdown, no explanations, max ${maxChars} characters.`
          },
 ...
 */
async function translateWithRetry(src, maxChars, apiKey, baseUrl, model, maxRetries) {
...
async function translatePromptsToEnglish(prompts, options) {
  const maxChars = Math.max(80, Number(options?.maxChars || 420));
