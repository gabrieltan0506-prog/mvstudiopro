...
/**
 * Convert a platform_topic_frame blueprint (title/hook/copywriting/detailedScript/executionDetails)
 * into Markdown for exportInteractiveHtml. Covers both new (inline metadata) and historical
 * (snapshot-loaded) creations so the full script + copywriting is saved alongside the cover image.
 */
function platformTopicFrameToMarkdown(
  ...
  if (ed && typeof ed === "object") {
    ...
    const scriptSteps = safeArr(ed.stepByStepScript);
...
export const creationsRouter = router({
  ...
  exportInteractiveHtml: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Fallback for platform_topic_frame: the full blueprint (title/hook/copywriting/
      // detailedScript/executionDetails) lives in platformStrategicBlueprintSnapshots
      // keyed by sceneId. Look it up and convert to Markdown so users get the full
      // copywriting + script content, not just the cover image.
      if (!md && row.type === "platform_topic_frame") {
        const sceneId = String(meta.sceneId || meta.scene_id || "").trim();
        // Prefer inline metadata if the pipeline already stored it (new creations)
        ...
        if (inlineTitle || inlineCopy) {
          md = platformTopicFrameToMarkdown({
            title: inlineTitle,
            copywriting: inlineCopy,
            hook: "",
            detailedScript: "",
            executionDetails: undefined,
          }, row.title || "");
        } else if (sceneId) {
          // Historical creations: load from snapshot table
          try {
            const { platformStrategicBlueprintSnapshots } = await import("../../drizzle/schema-platform-strategic-blueprints.js");
            const { desc: descOp, eq: eqOp } = await import("drizzle-orm");
            const snapRows = await database
              .select({ blueprintsJson: platformStrategicBlueprintSnapshots.blueprintsJson })
              .from(platformStrategicBlueprintSnapshots)
              .where(eqOp(platformStrategicBlueprintSnapshots.userId, ctx.user.id))
              .orderBy(descOp(platformStrategicBlueprintSnapshots.updatedAt))
              .limit(1);
            ...
            if (rawJson) {
              ...
              if (hit) {
                md = platformTopicFrameToMarkdown(hit as any, row.title || "");
              }
            }
          } catch (e) {
