 ...
 * 接入阿里雲 SMS / 騰讯雲 SMS 后，取消 TODO 註解并填入实现即可。
 *
 * 環境變量（接入時填入）：
...
// ─── In-memory store（與郵件 OTP 相同结構，接入時可複用） ───
...
export function registerSmsAuthRoutes(app: Express) {
   ...
   *
   * 目前狀态：501 Not Implemented（路由預備，待接入简讯 API）
   */
  app.post("/api/auth/send-sms-otp", (req, res) => {
    ...
    //   return res.status(429).json({ error: "该手機號请求過於频繁，请 10 分鐘后再试" });
    // }
  ...
  app.post("/api/auth/verify-sms-otp", (req, res) => {
    ...
    //   return res.status(400).json({ error: "驗證码已過期，请重新发送" });
    // }
    // if (record.attempts >= SMS_OTP_MAX_ATTEMPTS) {
    //   smsOtpStore.delete(phone);
    //   return res.status(429).json({ error: "驗證失敗次数過多，请重新发送" });
    // }
    // record.attempts += 1;
    // if (record.code !== otp) {
    //   const remaining = SMS_OTP_MAX_ATTEMPTS - record.attempts;
    //   return res.status(400).json({ error: `驗證码錯誤，還可嘗试 ${remaining} 次` });
    // }
    // smsOtpStore.delete(phone);
    // ... 查詢或创建用戶，创建 session，返回 { ok: true }
