...
import { getPlatformTrendReportCredits } from "../shared/platformTrendPricing.js";
...
7. nextMove（建议动作）：必须明确说出「发什么内容」与「如何开头」两件事。禁止写"先发一版内容拿反馈"这种空话。必须写出：具体的内容标题/主题 + 第一句话怎么说。标题须高反差/反常识/数字拧巴（对标「十碗饭反而瘦十斤」「打游戏怎么考上北大」）；例如：「发布《他天天打游戏，放榜怎么考上了北大？》，开头说：『不是天赋——是第 2 个习惯把分数拧回来了。』」开头须点名受众痛点并留半成品解法空间。
...
const PLATFORM_STAGE2_VOICE_GUIDANCE = `【口吻与生命力·第一优先】你写的是**给人愿意看完、愿意点开**的稿子，不是合规摘要，也不是说教课，更不是「概念+三点清单」顾问 PPT。
- **感情与共鸣硬门槛（每条必要）**：每条 title / hook / copywriting / detailedScript **必须**有情绪温度与共鸣触点（「就是我」/心疼/委屈/解气/不信感之一），像朋友在说话。干巴巴、正确但催眠 → 必须重写。
- **案例看情况加入（非每条强制）**：讲**疾病/慢病风险/误区**时，建议嵌入生动故事钉子（猝不及防、年轻中招如渐冻症等可含但不限、身边对照）——知识人人听过，故事才钉得住。讲**器官功能/机制科普**时，拟人、尺度具象、生活对照即可，**不必**硬塞真人病例。清单（2–3 点、最多 5）完全可用，但不能只有清单零感情。
- **同批禁复读**：六条须换不同场景/情绪/开头；禁止六条都是「高压中年缺恢复」空心换皮（808 批次为反面教材）。
- **痛点与爽点：黄金三秒先抛火花、结尾才说完整**：hook / 前 3 秒须同时出现痛点火花（情绪触点即可，不必须是病例）+ 爽点苗头；中段半成品展开；**禁止**痛点或爽点整段留到最后才第一次出现，也**禁止**开头就把完整方案说尽。结尾收满爽点 + CTA。
- **数目克制 + 数字钩子**：半成品优先 2–3 点，清单最多 5 点；可用「五个妙招先说第三个」拉完播。
- **取材开放且人物轮换**：热播/宵夜通勤亲子优先；古典可写但**同人默认 1 条**；禁止默认苏轼，三苏与唐宋八大家轮流；已出现的人本批勿再写（除非时事/季节/用户点名）。
- **去无聊硬门槛（雪糕公式）**：观众是来放松的，不是来听课。每条须对齐：**一眼懂的生活局 → 痛点1（猫腻/反常识）→ 痛点2（后果钉子）→ 以后会选/会做**。正例：不能给孩子一直吃雪糕 → 配料有猫腻 → 食欲下降影响发育 → 以后懂选雪糕。反例：把论文/指南结果「讲给大家听」。标题若像「读《××》领悟…」「哈佛实验室揭秘」「××观/管理/实验」或堆机制名词却无生活场面 → **必须重写**。
- **反面教材**：条条苏轼冷灶 × 血管/皮质醇；只有《藏海传》好看——下一批禁止再坍缩。
...
- **审核友好**：故事用于唤醒对照，不给观众下诊断、不开药、不承诺疗效。
...
1. contentBlueprints：须恰好包含 **6** 个具体可执行的内容方案，并与上方 **6** 个维度一一对应（第 1 条对应维度 1，依此类推，第 6 条对应维度 6）。每个方案须包含：
   - title（选题标题：**必须**高反差/反认知/反常识/猎奇缺口；气质对标「十碗饭反而瘦十斤」「打游戏怎么考上北大」；约12–28字可带悬念；禁止正确但无聊的百科题；禁止空心概念换皮且零共鸣）
   ...
   - hook（开头文案钩子：**必须**黄金三秒级停滑——**感情共鸣触点** + 爽点前置同句或紧挨；反问/物件/窘境；疾病向可加故事钉子，器官向可用拟人/对照；建议≥30字仍口语；禁先讲定义或说教开场；禁干巴巴零感情）
   - copywriting（核心文案方向，**建议**完整正文不少于200字。结构须含：**开场共鸣+爽点** → 点名目标客户 → 展开 → **2–3 个半成品解法要点（故意留白完整方案）** → **结尾咨询/私信/预约类 CTA**。口吻**幽默风趣、有感情**；清单可用；疾病知识点建议加故事，器官机制可用拟人不必硬塞病例；图文与视频均给出可直接使用的正文）
  ...
  "summary": "string"
    ...
    createPlatformAnalysisJob: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // ── 扣费真源：9 月 15 日前 50；北京时间 9 月 16 日起 60 ─────────
        let creditsBilled = 0;
        if (ctx.user?.id) {
          const isAdminUser = ctx.user.role === "admin" || ctx.user.role === "supervisor";
          if (!isAdminUser) {
            const cost = getPlatformTrendReportCredits(input.windowDays);
            const creditsInfo = await getCredits(ctx.user.id);
            if (creditsInfo.totalAvailable < cost) {
              throw new Error(
    ...
    generateVisualReport: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const systemPrompt = `你是一位顶级的新媒体数据分析师，专注于分析已采集内容的表现与赛道样本变化。今天是 ${currentDateStr}（时间一律按 Asia/Shanghai 北京时间 UTC+8）。
...
【赛道口径 — 须与历史分类对齐】
下方「行业样本推断」JSON 的每个 **key** 来自趋势样本入库时的 **正式分类**（industryLabels / contentLabels；缺省时为与增长评分一致的规则化大类兜底），**不是**从标题或评论里切出来的热词碎片。trackGrowth[].name **必须逐字使用该 JSON 的某一完整 key，不得组合、扩写或缩短分类名**。**不建议**使用与表中 key 无语义包含/对应关系的碎词、纯地名或账号梗充当整条赛道名。

【行业样本推断（近 ${wd} 天 vs 前 ${wd} 天，Asia/Shanghai 连续日对照）：**有前窗对照**用样本量推算增速，表中 **至 +100% 写「+N%」**；**若推算超过 +100% 则该 key 对应文案必须为「高热」**，禁止写百分比或「增长X倍」。**无前窗对照**必须写「缺少对照」，不得将样本排序换算成百分比。热门赛道列表不得含负向样本桶。trackGrowth.growth 必须与下表一致；禁止 N/A 与长句】
...
4. blueOceanWords：**选题关键词（分级，仅沿用历史字段名）**，每个平台 2-4 组，每组格式为 { "primary": "一级搜索词（2–12字）", "secondary": ["二级词1", "二级词2", "二级词3"]}。优先：用户意图清晰、可发笔记/短视频的长尾搜索词。一级词须从 platformEvidence.topTitles、**dramaMixNames / dramaRising（AI漫剧合集）**、行业样本 key、hotTopics 提炼；二级词可为更细切口（题材/人设/爽点）。抖音若有 dramaRising，至少 1 组 primary 贴近漫剧/短剧选题。**禁止输出空数组**；无法核实搜索量时仍须输出 2–4 组，不得写「尚未检索到」类占位句。
...
- trackGrowth：从上表选择最多8条完整分类，数量不得超过实际可用分类，不足时按实际数量输出；保留非负变化及「缺少对照」观察项（服务端会剔除负增长/无匹配）；**growth** 与下表完全一致：**+100% 及以下写「+N%」**；**超过 +100% 的格子表里会是「高热」，你必须写「高热」**，禁止自造百分比或倍数。**name** 与上表 JSON 的 key 对齐（见「赛道口径」）。勿编造。**严禁** N/A、括号长句。
...
- globalBlueOceanWords：**【必须输出 4–6 组，禁止空数组】** 聚合选定平台的选题关键词，一/二级分级。这些是创作建议，不代表已核实搜索量或低竞争。格式：[{"primary":"一级词","secondary":["二级词1","二级词2"]}]。须从 platformEvidence.topTitles、dramaMixNames、dramaRising、行业样本 key、各平台 hotTopics 提炼；含抖音漫剧样本时可输出「AI漫剧/重生漫剧」类一级词。无法核实月搜索量时仍须输出，**禁止**「尚未检索到蓝海词」等空话。
【证据口径】百分比仅表示已采集赛道样本条数对前一同长度窗口的变化，不是播放量、平台全量流量或算法推荐率；采集覆盖可能不同，本期含今日未结束时段。无对照不推算增长。关键词统一称为「选题关键词」，不得称低竞争蓝海或宣称搜索量；算法相关判断统一为「内容表现观察」，不得宣称已验证算法机制。
        ...
        try {
          ...
          const platformDetails = Array.isArray(parsed.platformDetails)
            ? parsed.platformDetails.map((p: any) => {
                const hotTopics = reconcilePlatformHotTopicsWithGlobalTrackGrowth(
                  Array.isArray(p?.hotTopics) ? p.hotTopics.map(safeStr) : [],
                  repairedTrackGrowth,
                  industryGrowthHintMap,
                );
                let blueOceanWords = normalizeBlueOceanWords(
                  p?.blueOceanWords ?? p?.blue_ocean_words ?? p?.["蓝海词"],
                );
                if (blueOceanWords.length === 0) {
                  blueOceanWords = buildEvidenceBlueOceanFallback({
                    platformDetails: [{ hotTopics }],
                    industryKeys: Object.keys(industryGrowthHintsObj || {}),
                    maxGroups: 4,
                  }).map((e) => ({ primary: e.primary, secondary: e.secondary }));
                }
                return {
          ...
          if (globalBlueOceanWords.length < 4) {
            const evidenceFill = buildEvidenceBlueOceanFallback({
            ...
            if (globalBlueOceanWords.length === 0) {
              globalBlueOceanWords = evidenceFill.length > 0
                ? evidenceFill
                : fallbackBlueOceanFromTracks(repairedTrackGrowth, platformDetails);
            } else {
              const seen = new Set(globalBlueOceanWords.map((b) => b.primary));
              for (const row of evidenceFill) {
                if (globalBlueOceanWords.length >= 6) break;
                if (seen.has(row.primary)) continue;
                seen.add(row.primary);
                globalBlueOceanWords.push(row);
              }
            }
          }
          ...
          const visualReportResult = {
            ...
            report: {
              reportTitle: `${input.platforms.map((platform) => PLATFORM_NAMES[platform] || platform).join("、")}趋势报告 · 近${wd}天 · ${pastStr} – ${todayStr}`,
              dateRange: `${pastStr} – ${todayStr}`,
              windowDays: wd,
          ...
          if (trendReportDeduct && !trendReportDeduct.alreadyCharged) {
            await refundCreditsForDeductAmount(
              ctx.user.id,
              `平台趋势报告生成失败退还：${message.slice(0, 120)}`,
              trendReportDeduct,
              "platformTrendRefund",
              { refundKey: `platform-trend-refund:${ctx.user.id}:${input.billingRequestId}` },
            ).catch((refundError) => {
          ...
          console.error("[generateVisualReport] failed:", message.slice(0, 400));
          // 独立功能：返回可读错误，供前端单独提示「趋势报表」失败原因（不 throw 以免 tRPC 吞掉细节）
          return {
            success: false,
            report: null,
