...
- **个案故事硬门槛（最高优先级）**：每条 title / hook / copywriting / detailedScript **必须**嵌入至少 1 个**生动具体的人的故事**（可含：前几天还说身体好、几天后猝不及防；年轻女孩渐冻症等「可包括但不限于」；饭桌劝父母失败的具体对话；同事红眼航班后倒下……）。**知识人人都听过，案例不是人人知道**——先讲人，再落 1–2 个机制点。禁止只绕「恢复系统/节律/留白/爵士隐喻/内经金句」打转而无真人个案（808 批次为反面教材，禁止复刻）。
- **同批禁复读**：六条须换**不同个案主角与案例类型**；禁止六条都是「高压中年缺恢复」换皮。
- **痛点与爽点：黄金三秒先抛火花、结尾才说完整**：hook / 前 3 秒须同时出现个案一击（或出事感）+ 爽点苗头；中段半成品展开；**禁止**痛点或爽点整段留到最后才第一次出现，也**禁止**开头就把完整方案说尽。结尾收满爽点 + CTA。
- **数目克制 + 数字钩子**：半成品优先 2–3 点，清单最多 5 点；可用「五个妙招先说第三个」拉完播。
- **取材开放**：古今典籍/人物（含宋朝）、平民贵族、影视健康桥段、反差身份选题均可；同批勿六条复读同一主角。
- **去无聊硬门槛**：标题若读起来像「××观 / ××管理 / 用 A 与 B 观察 C」且无个案/无画面 → **必须重写**。每条标题须含物件、身体动作、不信感、身份反差或**故事钩**之一。
- **完播画面**：detailedScript / 封面须提示**多元表情、可夸张姿势**（错愕、坏笑、失衡、发球定格等轮换），禁止六条端坐上课脸。
- **每条方案应有辨识度**：六条的标题、场景、情绪基调、开头句式、个案 **必须明显不同**。
- **具体优先于正确**：数字、道具、场所、第一句话怎么开口——**越能直接开拍越好**；允许适度文学化与比喻，但比喻不能替代个案。
- **coverHeadline**：8–14 字停滑短句；**禁止**把长 title 原样当封面主句。
- **审核友好**：个案用于唤醒对照，不给观众下诊断、不开药、不承诺疗效。
...
1. contentBlueprints：须恰好包含 **6** 个具体可执行的内容方案，并与上方 **6** 个维度一一对应（第 1 条对应维度 1，依此类推，第 6 条对应维度 6）。每个方案须包含：
   - title（选题标题：**必须**有好奇缺口/反常识/反差/**个案故事钩**之一，具体有画面；禁止正确但无聊的百科题；禁止「恢复系统/节律/留白」空心概念题且无人名/无出事感）
   ...
   - hook（开头文案钩子：**必须**黄金三秒级停滑——**个案一击或出事感** + 爽点前置同句或紧挨；反问/物件/窘境；建议≥30字仍口语；禁先讲定义或说教开场；禁只有概念无真人故事）
   - copywriting（核心文案方向，**建议**完整正文不少于200字。结构须含：**开场个案+爽点** → 点名目标客户 → 把故事讲满 → **2–3 个半成品解法要点（故意留白完整方案）** → **结尾咨询/私信/预约类 CTA**。口吻**幽默风趣、禁训话/公文**；**禁止**通篇「概念+三点清单」无个案；图文与视频均给出可直接使用的正文）
