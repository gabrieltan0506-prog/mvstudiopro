  ...
  storyboard: router({
    generate: protectedProcedure
      .input(z.object({
        ...
        sceneCount: z.number().min(1).max(15).default(5),
      }))
      .mutation(async ({ input, ctx }) => {
        const userId = ctx.user.id;
...
请确保生成的分镜脚本专业、详细、具有电影感，能够直接用于视频拍摄指导。`
    ...
    exportPDF: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Chinese font paths for PDF generation
        const CJK_FONT_REGULAR = '/usr/share/fonts/opentype/noto/NotoSansCJKsc-Regular.otf';
        const CJK_FONT_BOLD = '/usr/share/fonts/opentype/noto/NotoSansCJKsc-Bold.otf';

        // ─── Word 导出 ───────────────────────────────────
        if (input.format === "word") {
          // 生成 HTML 格式的 Word 文档（.doc）
          let html = `<!DOCTYPE html>
<html><head><meta charset="utf-8">
<style>
body { font-family: 'Microsoft YaHei', 'PingFang SC', 'Noto Sans SC', Arial, sans-serif; margin: 40px; color: #333; }
h1 { text-align: center; color: #1a1a1a; font-size: 28px; margin-bottom: 20px; }
h2 { color: #0a7ea4; font-size: 20px; border-bottom: 2px solid #0a7ea4; padding-bottom: 5px; margin-top: 30px; }
h3 { color: #444; font-size: 16px; margin-top: 15px; }
.info-row { display: inline-block; background: #e8f4f8; padding: 4px 12px; border-radius: 12px; margin: 3px 5px 3px 0; font-size: 13px; color: #0a7ea4; }
.scene-card { border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin: 15px 0; page-break-inside: avoid; }
.label { color: #888; font-size: 12px; margin-bottom: 4px; }
.tag { display: inline-block; background: #e8f4f8; padding: 2px 8px; border-radius: 4px; font-size: 12px; color: #0a7ea4; margin: 2px 4px 2px 0; }
img { max-width: 100%; border-radius: 8px; margin: 10px 0; }
.summary { background: #f8f9fa; padding: 20px; border-radius: 12px; margin-top: 20px; }
</style></head><body>`;

          html += `<h1>${input.storyboard.title}</h1>`;
          html += `<div style="text-align:center;margin-bottom:20px;">`;
          html += `<span class="info-row">BPM: ${input.storyboard.musicInfo.bpm}</span>`;
          html += `<span class="info-row">${input.storyboard.musicInfo.emotion}</span>`;
          html += `<span class="info-row">${input.storyboard.musicInfo.style}</span>`;
          html += `<span class="info-row">调性: ${input.storyboard.musicInfo.key}</span>`;
          html += `</div>`;

          for (const scene of input.storyboard.scenes) {
            html += `<div class="scene-card">`;
            html += `<h2>场景 ${scene.sceneNumber} <span style="font-size:14px;color:#888;font-weight:normal;float:right;">${scene.timestamp} (${scene.duration})</span></h2>`;
            if (scene.previewImageUrl) {
              html += `<img src="${scene.previewImageUrl}" alt="场景 ${scene.sceneNumber}" />`;
            }
            html += `<p class="label">场景描述</p><p>${scene.description}</p>`;
            html += `<p class="label">镜头运动</p><p>${scene.cameraMovement}</p>`;
            html += `<p class="label">情绪氛围</p><p>${scene.mood}</p>`;
            html += `<p class="label">视觉元素</p><p>${scene.visualElements.map(e => `<span class="tag">${e}</span>`).join('')}</p>`;
            if (scene.transition) {
              html += `<p class="label">转场建议</p><p>${scene.transition}</p>`;
            }
            html += `</div>`;
          }

          html += `<div class="summary"><h2>整体建议</h2><p>${input.storyboard.summary}</p></div>`;
          html += `</body></html>`;

          const wordFileName = `storyboard_${Date.now()}.doc`;
          const wordFilePath = path.join('/tmp', wordFileName);
          fs.writeFileSync(wordFilePath, html, 'utf-8');

          const wordBuffer = fs.readFileSync(wordFilePath);
          const { key: wordKey } = await storagePut(wordFileName, wordBuffer, 'application/msword');
          fs.unlinkSync(wordFilePath);
          // Get publicly accessible download URL
          const { url: wordDownloadUrl } = await storageGet(wordKey);
          console.log('[exportPDF] Word generated:', { format: 'word', key: wordKey, url: wordDownloadUrl });

          return {
            success: true,
            pdfUrl: wordDownloadUrl,
            message: 'Word 文档已生成！'
          };
        }
        ...
        const fontBold = (size: number) => doc.fontSize(size).font(CJK_FONT_BOLD);
        const fontRegular = (size: number) => doc.fontSize(size).font(CJK_FONT_REGULAR);
        
        // Title
        fontBold(24).text(input.storyboard.title, { align: 'center' });
        doc.moveDown();
        
        // Music Info
        fontBold(14).text('音乐信息', { underline: true });
        fontRegular(12);
        doc.text(`BPM: ${input.storyboard.musicInfo.bpm}`);
        doc.text(`情感基调: ${input.storyboard.musicInfo.emotion}`);
        doc.text(`音乐风格: ${input.storyboard.musicInfo.style}`);
        doc.text(`调性: ${input.storyboard.musicInfo.key}`);
        doc.moveDown();
        ...
        for (const scene of input.storyboard.scenes) {
          ...
          if (scene.previewImageUrl) {
            try {
              const response = await axios.get(scene.previewImageUrl, { responseType: 'arraybuffer', timeout: 15000 });
        ...
        doc.moveDown();
        fontRegular(11).text(input.storyboard.summary);
        ...
        console.log('[exportPDF] PDF generated:', { format: 'pdf', key: pdfKey, url: pdfDownloadUrl });
