 ...
 * - **雙條（可選）**：{@link runCoverDeepResearchDualBatchBrief} 對**兩則選題**各開一次 Interaction（首輪並行）；{@link runCoverDeepResearchBriefPreferDual} 在兩條皆無有效簡報時回傳 **null**，調用方**不注入** DR 正文，改由**單題**文案直送 **GPT 5.4** 英文化（再出封面／分鏡等）。不得把「兩組無關選題」塞進**同一則** GPT 翻譯請求。
...
/** 雙條並行時每條獨立輪詢預算：預設為單條上限的 2 倍（仍 ≤10min）。 */
export function resolveDualPerSlotTimeoutMs(): number {
  const base = resolveTimeoutMs();
  return Math.min(600_000, Math.max(30_000, base * 2));
}
...
/**
 * 有第二則選題時：先 {@link runCoverDeepResearchDualBatchBrief}；**兩條皆無**有效简報則回傳 **null**（下阶只用主選題語境 + GPT 5.4，不注入 DR）。
 * 僅主題時：等同單次 {@link runCoverDeepResearchInteractionsBrief}。
 */
export async function runCoverDeepResearchBriefPreferDual(
  ...
  if (a) {
    note("採用條 A（主選題）DR 簡報");
    return a;
  }
  if (b) {
    note("條 A 無效 · 採用條 B（副選題）DR 簡報");
    return b;
  }
  note("兩條 DR-Pro 均无有效简報 · 不注入 DR · 下階單題 + GPT 5.4 英文化");
  return null;
}
