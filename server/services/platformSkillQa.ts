...
function buildManhuaEngineFactsBlock(context: ManhuaCreativeAdvisorContext): string {
  ...
  return [
    ...
    "生成视频提示词或优化建议时，先说明用户所选模型，再按当前镜头给出可直接采用的修改稿；指出本次相关的时长、参考数量与引用写法，不罗列无关参数。",
    "需求超过上述模型能力时，明确告知当前模型不支持，并建议更换支持该需求的模型；不要求截断对白、丢弃参考素材或自动改写剧情来迁就模型，也不承诺更低费用获得相同能力。",
    "建议仅供用户选择，不自动修改工程或提交生成；未知的分辨率、价格及供应商能力不得猜测。",
