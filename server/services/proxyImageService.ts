...
/**
 * 單次 gpt-image HTTP 等待上限。
 * 曾誤用 `Math.min(300_000, env)`，導致 `GPT_IMAGE_FETCH_TIMEOUT_MS` 無論設多大都被壓成 5min，`CLIENT_FETCH_ABORT_TIMEOUT` 只能狂重試。
 * - 預設仍 5min；設 `GPT_IMAGE_FETCH_TIMEOUT_MS` 可拉高至本 ceiling（預設 13min，低於 worker `platform_topic_image` 15min 以免整 job 先死）。
 * - 必要時 `GPT_IMAGE_FETCH_TIMEOUT_MAX_MS`（300_000～900_000 ms）覆寫 ceiling。
 */
const GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING = (() => {
  const raw = Number(process.env.GPT_IMAGE_FETCH_TIMEOUT_MAX_MS);
  if (Number.isFinite(raw) && raw >= 300_000) return Math.min(Math.floor(raw), 900_000);
  return 780_000;
})();
const GPT_IMAGE2_REQUEST_TIMEOUT_MS = Math.min(
  GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING,
  Math.max(60_000, Number(process.env.GPT_IMAGE_FETCH_TIMEOUT_MS) || 300_000),
);

/**
 * 平台頁 **封面單幀 / 2×4 分鏡**：OhMyGPT 排隊常超過全域預設 5min，第三次同尺寸重試才成功多半是前兩次被本地 Abort 斷開。
 * 預設單次 HTTP **10min**；可用 `PLATFORM_TOPIC_GPT_IMAGE_TIMEOUT_MS`（60_000～與 {@link GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING} 齊）覆寫。
 */
export function resolvePlatformPageGptImageRequestTimeoutMs(): number {
  const raw = Number(process.env.PLATFORM_TOPIC_GPT_IMAGE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 60_000) {
    return Math.min(Math.floor(raw), GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING);
  }
  return Math.min(600_000, GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING);
}
...
function classifyGptImage2FetchException(e: unknown, timeoutMsUsed?: number): { code: string; detail: string } {
  const msg = e instanceof Error ? e.message : String(e);
  const name =
    typeof (e as { name?: string })?.name === "string" ? String((e as { name: string }).name) : "";
  const ms = timeoutMsUsed ?? GPT_IMAGE2_REQUEST_TIMEOUT_MS;

  if (name === "TimeoutError" || /aborted due to timeout|The operation was aborted/i.test(msg)) {
    return {
      code: "CLIENT_FETCH_ABORT_TIMEOUT",
      detail: `本地 fetch 在 ${ms}ms 内未收齐响应（AbortSignal.timeout）。常见：上游仍在生图、代理排队、网关慢。并非响应体里的 OpenAI error JSON；重试会再发一整条 HTTP 请求，可能多计费。`,
    };
  }
...
async function postGptImage2AndUpload(
  ...
  opts: {
    ...
    flowLog?: string[];
    /** 覆寫單次 fetch 逾時；默認 {@link GPT_IMAGE2_REQUEST_TIMEOUT_MS} */
  ...
  const fetchTimeoutMs = Math.min(
    GPT_IMAGE2_REQUEST_TIMEOUT_MS_CEILING,
    Math.max(60_000, opts.requestTimeoutMs ?? GPT_IMAGE2_REQUEST_TIMEOUT_MS),
  );
  ...
  for (const size of sizes) {
    ...
    while (attempt < perSizeAttemptCap) {
      ...
      try {
        ...
        const { code, detail } = classifyGptImage2FetchException(e, fetchTimeoutMs);
        appendImageFlowLog(
          L,
...
export async function generateGptImage2FromRawEnglishPrompt(options: {
  ...
  flowLog?: string[];
  /** 平台封面/分鏡：可拉長單次 OhMyGPT 等待；默認與全域 {@link GPT_IMAGE2_REQUEST_TIMEOUT_MS} 一致 */
  gptImageRequestTimeoutMs?: number;
}): Promise<string | null> {
  ...
  appendImageFlowLog(
    L,
    `[单帧主路径] GPT-IMAGE-2（OhMyGPT）· ${options.aspectRatio} · 单请求超时 ${options.gptImageRequestTimeoutMs ?? GPT_IMAGE2_REQUEST_TIMEOUT_MS}ms · 试尺寸序列: ${sizes.join(" → ")} · 英文 prompt 约 ${prompt.length} 字`,
  );
  const primary = await postGptImage2AndUpload(prompt, options.gcsSubdir, {
    sizes,
    flowLog: L,
    requestTimeoutMs: options.gptImageRequestTimeoutMs,
  });
  if (primary) {
...
export async function generatePlatformCompositeSheetImage(options: {
  ...
  for (let attempt = 1; attempt <= compositeMaxAttempts; attempt++) {
    ...
    try {
      ...
      appendImageFlowLog(
        L,
        `[2×4·步骤2] GPT-IMAGE-2 宽幅横版 · gcsSubdir=${subdir} · 单请求超时 ${resolvePlatformPageGptImageRequestTimeoutMs()}ms · 尺寸序列（OpenAI gpt-image 白名单）: ${GPT_IMAGE2_LANDSCAPE_SIZES.join(" → ")}`,
      );
      const primary = await postGptImage2AndUpload(promptForImage, subdir, {
        sizes: GPT_IMAGE2_LANDSCAPE_SIZES,
        flowLog: L,
        requestTimeoutMs: resolvePlatformPageGptImageRequestTimeoutMs(),
      });
      if (primary) {
...
export async function generateImageGpt2WithImagenFallback(options: {
  ...
  appendImageFlowLog(
    L,
    `[版式兜底] buildTypographyImagePrompt + GPT-IMAGE-2 · mode=${options.mode} · 9:16（画内零字策略）· 单请求超时 ${options.requestTimeoutMs ?? GPT_IMAGE2_REQUEST_TIMEOUT_MS}ms`,
  );
  const primary = await generateGptImage2({
    title: options.title,
    copywriting: options.copywriting,
    mode: options.mode,
    isTrial: options.isTrial,
