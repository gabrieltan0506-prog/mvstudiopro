...
const GPT_IMAGE2_STORYBOARD_2X4_PIXEL_LOCK =
  "CRITICAL COMPOSITION LOCK: single wide landscape ~16:9 master. TOP ~8–12% HEIGHT ONLY: full-width band = **内容总结** as the sheet-level theme (whole-script / episode summary); may include 「· 分镜脚本」suffix—**no per-shot titles in this band**. Below: EXACTLY eight equal panels, 2 rows × 4 columns, straight gutters. PER PANEL top-to-bottom: (1) **分镜主题描述** one bold legible **Simplified Chinese** line for that shot only; (2) cinematic still (dark/moody stills OK—do not bleach the cinematography); (3) bottom ~25–30% = **Simplified Chinese** table with **four labeled fields** — **景别**, **运镜**, **画面内容**, **台词与音效** — **light paper fill** (cream / warm off‑white / light warm‑gray parchment) + **near‑black glyphs** inside cells + thin dark grid lines—**readable caption band** clearly separated from the still above (hairline or margin OK). TABLE FORBIDDEN: full‑bleed **black footer** with skinny **reversed white** micro‑type dens Chinese (hard‑to‑read UI). SHEET FORBIDDEN: missing top **内容总结** strip; first row of panels flush to canvas top; placing per-shot **分镜主题描述** in the global top band instead of inside each cell; fewer than eight cells; English-only tables; wholly empty panels.";
...
- BELOW: EXACTLY eight equal panels, 2 rows × 4 columns, straight gutters. Each panel **top → bottom**: (1) one bold **分镜主题描述** line in 简体中文 for that shot only; (2) cinematic still (dark/night moods OK above the table); (3) lower ~25–30% = **Simplified Chinese** table with **four fields**: **景别**、**运镜**、**画面内容**、**台词与音效** — all four with content; **table band MUST use a warm off‑white / cream / parchment / light‑warm‑gray paper fill** + **near‑black table text**, thin dark gridlines; optionally a slim separator between still and table. Table body must be 简体中文.
- **Legibility:** avoid **dense white glyphs on jet‑black datasheet bands** inside each cell (hard to read); caption zones should read like **light paper + dark type**.
