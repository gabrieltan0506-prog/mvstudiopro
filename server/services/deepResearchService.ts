...
const CONSUMER_COVER_QUALITY_TAIL =
...
// Max 版：战略智库整份白皮书 plan+execute，单次常需 30 分钟量级；勿用于封面短研。
...
/** 超高点击率封面：Pro 竞品清洗本地轮询上限（默认 8 分钟；`COVER_DEEP_RESEARCH_POLL_MAX_MS` 可覆盖） */
const COVER_DEEP_RESEARCH_POLL_DEFAULT_MS = 8 * 60 * 1000;
 ...
 */
export async function runCoverCompetitorDeepResearchBrief(params: {
  topicHook: string;
  appealHook: string;
  context: string;
  format: "短视频" | "图文";
  ...
  const flag = String(process.env.COVER_DEEP_RESEARCH_ENABLED ?? "1").trim().toLowerCase();
  if (flag === "0" || flag === "false") {
    params.flowLog.push(`${new Date().toISOString()}  [Deep Research Pro] 已跳过（COVER_DEEP_RESEARCH_ENABLED）`);
    return null;
  }

  const apiKey = String(process.env.GEMINI_API_KEY || "").trim();
  if (!apiKey) {
    params.flowLog.push(`${new Date().toISOString()}  [Deep Research Pro] 跳过：缺少 GEMINI_API_KEY`);
    return null;
  }
  ...
  const appealHook = String(params.appealHook || "").trim().slice(0, 500);
  const context = String(params.context || "").trim().slice(0, 4000);
  const format = params.format;

  const systemBlock = `【全局系统指令 · 必须遵守 — 超高点击率封面 · 竞品短研】
本次不是企业战略白皮书。你是顶尖短视频/图文信息流增长与竞品洞察顾问。
权重：70% 公开渠道可验证的爆款封面/标题/划停动机（抖音、小红书、B站、快手及同类信息流），30% 受众心理与品类语境。
必须联网检索近 12–24 个月可引用的公开素材；无法核实的数据须标注【推断】，禁止伪造链接。
输出仅限简体中文 Markdown，约 400～2200 字。禁止输出「战略摘要」「财务模型」「长篇政策解读」类商用研报章节。
必须包含这些章节（用 ## 小标题）：
1) 对标标题角度（3～5 条极短句，可化用）
2) 高点击率封面视觉共性（对比度、主体占比、文字层、情绪）
3) 划停关键词与内容避雷
4) 与本选题的差异化卡位（1～3 句）`;

  const userBlock = `【用户选题主句】
${topicHook}
...
请在限定时间内完成检索与推理，直接输出 Markdown 结果正文。`;

  const fullInput = `${systemBlock}\n\n---\n\n【用户课题】\n${userBlock}`;
  ...
  const flushProgressLines = async () => {
    const jid = params.progressJobId;
    if (!jid) return;
    try {
      const { patchJobRunningProgress } = await import("../jobs/repository.js");
      await patchJobRunningProgress(jid, {
        imageGenFlowLog: params.flowLog.slice(-240),
  ...
  try {
    const outputs = await pollInteraction(
      interactionId,
      apiHeaders,
      maxPollMs,
      undefined,
...
export async function ensureCoverForCreation(
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 2 && !imageUrl; attempt++) {
      try {
        ...
        imageUrl = await generateImageViaGeminiApiKey({
          prompt: flashPrompt,
          aspectRatio: "9:16",
          model: STRATEGIC_FALLBACK_FLASH_MODEL,
          imageSize: "2K",
        });
        console.log(
          `[ensureCoverForCreation] ✅ on-demand 兜底 Consumer Nano Banana 2 · 2K（${STRATEGIC_FALLBACK_FLASH_MODEL}），第 ${attempt}/2 次 creationId=${creationId}`,
        );
      } catch (e: any) {
        console.warn(
          `[ensureCoverForCreation] ⚠️ 兜底 Consumer 2K 第 ${attempt}/2 次失败: ${e?.message ?? e} creationId=${creationId}`,
        );
        if (attempt < 2) await sleep(1000);
      }
    }
    ...
    if (!imageUrl) {
      console.warn(`[ensureCoverForCreation] Fly/Vertex 与 Consumer 2K 均失败 creationId=${creationId}，模板将走文字框回退`);
      return undefined;
    }
    ...
    const coverPromise: Promise<string | undefined> = (async () => {
      ...
      for (let i = 1; i <= 2 && !cover; i++) {
        try {
          ...
          console.log(
            `[deepResearch][async-cover] ✅ 兜底 Consumer Nano Banana 2 · 2K（${STRATEGIC_FALLBACK_FLASH_MODEL}），第 ${i}/2 次 jobId=${jobId}`,
          );
        } catch (e: unknown) {
          const msg = e instanceof Error ? e.message : String(e);
          console.warn(`[deepResearch][async-cover] ⚠️ 兜底 Consumer 2K 第 ${i}/2 次失败：${msg} jobId=${jobId}`);
          if (i < 2) await sleep(2000);
        }
      }
...
const STRICT_SCENE_CONSTRAINT = `
CRITICAL INSTRUCTION FOR SCENE GENERATION:
You MUST generate exactly 5 to 7 scene descriptions based on the markdown content.
Do NOT generate less than 5 scenes under any circumstances.
Ensure the scenes are evenly distributed across the logical structure of the report.
`.trim();
...
${reportSnippet}`,
  ...
  // ── 解析 LLM 输出：先严格 parse；**僅在 0 場景時** retry；不再因 < minCount 整批放棄配圖。
  let parsed = parseScenesJson(sceneJsonText);
  const firstSceneCount = parsed?.scenes?.length ?? 0;
  if (firstSceneCount > 0 && firstSceneCount < minCount) {
    console.warn(
      `[deepResearch][scenes] 第 1 次解析得 ${firstSceneCount} 个场景（目标 ${minCount}-${maxCount}），仍繼續生圖（不因數量放棄整批）。`,
    );
  }
  if (!parsed || firstSceneCount === 0) {
    console.warn(`[deepResearch][scenes] 第 1 次解析无场景，发起精简 retry…`);
...
${reportSnippet.slice(0, 8000)}`,
    ...
    if (retryParsed && (retryParsed.scenes?.length ?? 0) > 0) {
      parsed = retryParsed;
    }
  }

  if (!parsed || (parsed.scenes?.length ?? 0) === 0) {
    console.warn(`[deepResearch][scenes] 警告：模型未產出場景，跳過配圖注入。`);
    return [];
  }
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (i < scenes.length - 1) {
      console.log(`[deepResearch][scenes] 進入 3 秒冷卻，避免觸發 Vertex HTTP 429...`);
      await sleep(3000);
    }
  }
