 ...
 * This significantly improves 3D model quality by isolating the subject.
 */
export async function removeBackground(imageUrl: string): Promise<string> {
  ...
  try {
    ...
    if (!outputUrl) {
      console.error("[BiRefNet] No output image URL:", JSON.stringify(data, null, 2));
      throw new Error("去背景失败");
    }

    console.log(`[BiRefNet] Background removed in ${timeTaken}s`);
    return outputUrl;
  } catch (error: any) {
    const timeTaken = ((Date.now() - startTime) / 1000).toFixed(1);
...
async function ensureAccessibleUrl(imageUrl: string): Promise<string> {
  ...
  console.log("[Hunyuan3D] 原始 URL:", imageUrl.substring(0, 100) + "...");
...
export async function generate3DModel(input: Hunyuan3DInput): Promise<Hunyuan3DTask> {
  ...
  try {
    ...
    // 解析返回结果 — fal.ai 实际返回结构：
    // model_glb: { url } (可能实际是 OBJ), model_urls: { glb, fbx, obj }, texture, thumbnail, material_mtl
    console.log("[Hunyuan3D] 返回数据 keys:", Object.keys(data));
    ...
    if (error.message?.includes("credentials") || error.message?.includes("FAL_API_KEY")) {
