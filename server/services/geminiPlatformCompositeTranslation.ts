...
- **有人物时·表情姿势硬约束（压过「挺拔从容冷淡大片」默认）**：禁止正面端坐/端站上课脸、证件照微笑、对称讲课站姿。必须画成**一眼有事发生**的抓人瞬间——表情（大笑/错愕/陶醉眯眼/坏笑/夸张怀疑/吃瘪等）或动态姿势（跳跃/差点摔倒失衡/探身回头/端托盘侧身大步/歪头半遮脸等）至少占一项，随选题换，勿每张重复。时装质感可保留，但脸与身体先服务停滑点击欲。锁脸参考时仍须丰富表情姿势，禁止退化证件照。
