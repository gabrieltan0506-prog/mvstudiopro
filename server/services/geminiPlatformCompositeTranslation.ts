...
/**
 * **2×4 / 3×4 编导分镜图 / 小红书八格**（或 **3×4 分段横排**）的「中文直送主体」。
 * 仅产出**中文画面主体**；调用方仍会在其后拼接英文像素锁（`GPT_IMAGE2_*_2X4_PIXEL_LOCK` / `*_ROWBAND_*`）+ 顶栏注入 + 镜头/光影 modifier。
 * 视频向 = 编导分镜图（分镜格 + 导演板节奏）；见 `shared/bianDaoStoryboard.ts`。
 */
export function buildCompositeSheetDirectChineseBody(
  ...
  if (isStoryboard) {
    if (rowBand) {
      return `请直接据下方中文脚本生成**3×4 十二格长图中的一整横排编导分镜**（横版约 16:9，仅 **1 行 × 4 列共 4 格**，不要画成完整 2×4 八格）：
${BIAN_DAO_SHEET_BODY_DIRECTIVE_ZH}
- 严格排成单横排四格，格线笔直、格间留白清晰，左→右顺扫；本段将与其他横排纵向拼成 3×4 长图（本横排序号建议 ${badgeRange}，整表 01–12）；横排内须承接起—承—转—合之一拍，勿四格情绪互不相关。
- 每一格自上而下：① 本格编导分镜主题（一行加粗简体中文）；② 该镜头电影级写实剧照（高细节，约占 65–70%）；③ 格内底部约 30–35% 为简体中文六栏小表，表头固定【${STORYBOARD_PANEL_TABLE_FIELDS_ZH}】六栏都要填（含灯光安排与情绪表达）。
- 风格：电影感、8k、精致布光、统一高级色调；所有屏内文字一律**简体中文、印刷清晰、不可乱码/缺笔**。
${simpLockZh}
${STORYBOARD_ON_IMAGE_TEXT_ZH}
${STORYBOARD_LIGHTING_EMOTION_GUIDANCE_ZH}
- 若脚本含【编导分镜·导演板】【光影与机位约束·拍摄手法】或【上传素材拍摄技法】，四格的景别/运镜/布光/走位/情绪**强烈建议**对齐该约束（教学演示类优先固定中远景、前景操作物、背景大屏同步）。
...
${slice}`;
    ...
    return `请直接据下方中文脚本生成一张**电影级 2×4 八格编导分镜图**（横版约 16:9 单张主表，不是单张满版海报）：
${BIAN_DAO_SHEET_BODY_DIRECTIVE_ZH}
- 顶部约 8–12% 为通栏【内容总结】标题栏（简体中文·全片梗概 + 风格/节奏起承转合/情绪弧，不放各格分镜标题）。
- 其下严格排成 **2 行 × 4 列、共 8 格**，格线笔直、格间留白清晰，按 row1 左→右、row2 左→右顺扫；八格须读出导演板级情绪递进，而非互不关联静帧清单。
- 每一格自上而下：① 本格编导分镜主题（一行加粗简体中文）；② 该镜头电影级写实剧照（高细节，约占 65–70%）；③ 格内底部约 30–35% 为简体中文六栏小表，表头固定【${STORYBOARD_PANEL_TABLE_FIELDS_ZH}】六栏都要填（含灯光安排与情绪表达）。
- 风格：电影感、8k、精致布光、统一高级色调；所有屏内文字一律**简体中文、印刷清晰、不可乱码/缺笔**。
${simpLockZh}
${STORYBOARD_ON_IMAGE_TEXT_ZH}
${STORYBOARD_LIGHTING_EMOTION_GUIDANCE_ZH}
- 若脚本含【编导分镜·导演板】【光影与机位约束·拍摄手法】或【上传素材拍摄技法】，八格的景别/运镜/布光/走位/情绪**强烈建议**对齐该约束（教学演示类优先固定中远景、前景操作物、背景大屏同步）。
...
【停滑封面·高点击短钩】
- 主标题用**简体中文**，约 **10–18 字**，大而清晰、印刷级，**必须且只能**呈现「${hook}」；主句气质要对齐反差/反常识/猎奇缺口（数字拧巴、结果颠倒、信息悬念），禁止正确但无聊的论文题。
${sublineRule}
- **提亮杀伤字**：主句内仅 **2–6 个关键字**用**香槟金/暖琥珀**提亮；其余米白/象牙白；**忌**荧光粉+荧光绿廉价撞色；不要为「说完整」再堆说明书墙。
- **版式·网感张力（高级不俗）**：主信息仍是「${hook}」；允许左右竖排关键词夹人脸、背后竖排关键词、克制提亮块、主词**轻微变形/厚描边**增停滑（精致可读，忌塑料贴纸字）；禁止把「点开全文 / 救急大招 / 各地名单」等 CTA 或正文摘要印上封面。
- **禁止**百科封面：长段论述、履历堆砌、四条以上卖点清单、多图标+多辅标栏、说明书式信息图。
- **禁止**默认暗沉、低照度、压抑严肃、葬礼感光影；须**生活化、年轻化、健康化、杂志级高级感**；优先便利店/餐厅/家装/哄娃等烟火气场域，或偶用抽象冲击动作（同批勿滥用）。
- 优先自然窗光 / 户外或生活场域、明快克制配色 + **单一**强调色；有点击欲，不要死板正装肖像海报。
- **有人物时·文案档位=表情动作档位（硬）**：禁止望窗外发呆、证件照微笑、正襟危坐与夸张标题同框。必须**一眼有事发生**，且与主句同拍：吃喝/减脂/午餐向优先**张口大吃、鼓腮咀嚼、叉子怼嘴、满足或惊喜表情**；亦可错愕/坏笑/陶醉/探身递物/运动定格。同批姿势轮换，勿全坐着、勿全挤同一表情。
- **清单/攻略向封面（藍海如親子旅遊清單·对标 x1/x3）**：优先 **flat lay 神器墙 / 摊开行李箱 / 备忘录怼镜头 / 手持神器特写**；主句可用「必備｜清單｜保姆級｜不踩雷｜被問爆」杀伤词 + **大数字件数** + **结果钉**（如全程安静/少带/省事）；**禁止**默认正装讲课半身像。
- **场景服饰硬锁（防穿帮）**：动作决定服装——网球发球必须网球运动装（短袖/网球裙裤+球鞋+球拍），**禁止长大衣/西服外套打网球**；游泳用泳装；爬山用户外功能装；装箱/带娃用休闲出行装；厅堂访谈才可用西装/丝绸衬衫。锁脸时仍须丰富姿势，且衣着跟场景。
- 可借鉴小红书网感封面的「大字停滑、关键字提亮、侧栏/竖排张力」，但人物必须有戏且与文案同档（含张口大吃线），勿正坐上课脸。
- 场景随文案多样化、贴合选题；人物脸/上半身可读，主标题不压脸。
- **不要**为「显得专业」强行加 2–4 个线描图标栏；最多 0–1 个极克制点缀，且不可抢主句。
- 从语境中只保留 **1–3 个**最能辨识此人设的特色道具/场景锚点（如典籍、耳机、节拍器、旅行物件、带娃神器），**禁止**把语境关键词全部画进画面。
- masterpiece、8k、视觉冲击力强；屏内文字一律**中国大陆简体中文、清晰不乱码**（严禁繁体）。
