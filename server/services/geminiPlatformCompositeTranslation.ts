...
/**
 * 平台英文化 · Flash：**預設 `global`**（與 {@link DEFAULT_VERTEX_FLASH_TRANSLATION_MODEL} 在 Vertex 上可用區域一致）。
 * 可隨時以 `VERTEX_GEMINI_FLASH_TRANSLATION_LOCATION` 覆寫（例如改為區域端點做對照）。
 */
...
/** 平台单帧 / 批量封面 / 宽幅合成：**英文化**引擎（GPT 5.4 默认；`vertex_*` 为 Vertex Flash · 預設 global）。 */
...
/**
 * 舊名保留：平台「探索」英文化走 {@link callVertexGeminiFlashTranslation}（Flash · 預設 global）。
 */
...
/**
 * 平台 2×4 / 小紅書合成與選題單幀：預設 **GPT 5.4**；選 **Vertex 探索** 時走 **Flash（預設 global）**。
...
export async function translatePlatformCompositeToEnglishPrompt(options: {
  ...
  translator?: PlatformImagePromptTranslator;
  /** A/B：`gemini31flash` 強制走 Flash（預設 global）；`gpt54` 強制 GPT 5.4 */
