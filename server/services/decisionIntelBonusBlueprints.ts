...
- 每条须含：title、format（「短视频」或「图文」）、hook（≥30字）、copywriting（≥180字）、suitablePlatforms（字符串数组）、actionableSteps（≥3条）、detailedScript（≥280字）、publishingAdvice、highlightKeywords（字符串数组 2–6 个）、platformVariants（xiaohongshu/bilibili/weixin_channels，各含最多13字 coverHeadline 高点击短钩，超则精简）、executionDetails（含 environmentAndWardrobe、lightingAndCamera、stepByStepScript 数组≥4步）。
