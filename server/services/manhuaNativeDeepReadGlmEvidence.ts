...
export async function readNativeDeepReadGlmRecoveredEvidence(input: {
  ...
  const strictSchema = input.expectedRequestWithoutPreferredGateway.requireResponseJsonSchema === true
    || (requestPayload.request as Record<string, unknown>)?.requireResponseJsonSchema === true;
  if (canonicalJson(strictSchema ? requestPayload.request : stripSchema(requestPayload.request)) !== canonicalJson(strictSchema ? expectedRequest : stripSchema(expectedRequest))) {
