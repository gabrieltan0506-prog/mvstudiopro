...
describe("weixinChannelsMiner", () => {
  ...
  it("千条批次固定使用 GLM 5.3 High、JSON、65K，且不传采样参数（0911 换档：GLM 无 medium 档）", async () => {
    ...
    const job: FinalAnalysisJob = { jobId: "ds-1", kind: "formal", stage: "deepseek_batch", threshold: 1_000, rawCount: 1_000, locallyDedupedCount: 1_000, observationIds: [persisted.observationId], analysisObservationIds: [persisted.observationId], lunaBatchIds: [], status: "processing", terraModel: "z-ai/glm-5.3-flash", reasoningEffort: "medium", createdAt: "2026-08-14T00:00:00.000Z", updatedAt: "2026-08-14T00:00:00.000Z" };
    const output = await invokeWeixinChannelsDeepSeekBatch({ job, observations: [persisted], invoke: invoke as never });
    expect(invoke.mock.calls[0][0]).toMatchObject({
      modelName: "z-ai/glm-5.3-flash", reasoningEffort: "high", requestId: "ds-1",
  ...
  it("DeepSeek 截断或 schema 越界时只重试一次，并累计两次用量", async () => {
    ...
    const job: FinalAnalysisJob = { jobId: "ds-retry", kind: "formal", stage: "deepseek_batch", threshold: 1_000, rawCount: 1_000, locallyDedupedCount: 1_000, observationIds: [persisted.observationId], analysisObservationIds: [persisted.observationId], lunaBatchIds: [], status: "processing", terraModel: "z-ai/glm-5.3-flash", reasoningEffort: "medium", createdAt: "2026-08-14T00:00:00.000Z", updatedAt: "2026-08-14T00:00:00.000Z" };
    const output = await invokeWeixinChannelsDeepSeekBatch({ job, observations: [persisted], invoke: invoke as never });
  ...
  it("DeepSeek 新字段、枚举外值与超过四十字文本都不能进入下游", async () => {
    ...
    const invoke = vi.fn()
      ...
      .mockResolvedValueOnce({ provider: "DeepSeek", choices: [{ message: { content: JSON.stringify(invalid) }, finish_reason: "stop" }] });
    const persisted = persistableWeixinChannelsObservation(observation({ likes: 3_000, shares: 2_000 }));
    const job: FinalAnalysisJob = { jobId: "ds-invalid", kind: "formal", stage: "deepseek_batch", threshold: 1_000, rawCount: 1_000, locallyDedupedCount: 1_000, observationIds: [persisted.observationId], analysisObservationIds: [persisted.observationId], lunaBatchIds: [], status: "processing", terraModel: "z-ai/glm-5.3-flash", reasoningEffort: "medium", createdAt: "2026-08-14T00:00:00.000Z", updatedAt: "2026-08-14T00:00:00.000Z" };
