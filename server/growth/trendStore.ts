...
function dedupeTrendItems(existing: TrendItem[] = [], incoming: TrendItem[] = []) {
  ...
  for (const raw of [...existing, ...incoming]) {
    ...
    bucket.set(item.id, {
      ...
      favorites: Math.max(current.favorites || 0, item.favorites || 0) || undefined,
      followers: Math.max(current.followers || 0, item.followers || 0) || undefined,
      friendsFollowing: Math.max(current.friendsFollowing || 0, item.friendsFollowing || 0) || undefined,
...
export async function updateTrendBackfillProgress(progress: Partial<TrendBackfillProgress>) {
  ...
  ).filter((platform): platform is GrowthPlatform => BACKFILL_PLATFORMS.has(platform)).map((platform) => {
