...
describe("weixinChannelsMinerStore", () => {
  ...
  it("999 条不建任务，第 1000 条并发 ingest 也只建一个正式任务且单条模型调用为零", async () => {
    ...
    expect(state.jobs[0]).toMatchObject({ stage: "deepseek_batch", terraModel: "z-ai/glm-5.3-flash", threshold: 1_000 });
  ...
  it("5 条 probe 照常持久化，只调用一次 DeepSeek，且不计入正式额度", async () => {
    ...
    const invoke = vi.fn().mockResolvedValueOnce({ id: "deepseek", created: 1, model: "z-ai/glm-5.3-flash", provider: "DeepSeek", choices: [{ message: { content: JSON.stringify(result) } }] });
  ...
  it("累计八个千条 DeepSeek 结果后只创建一个 Terra 清洗任务，且不重新发送原始记录", async () => {
    ...
    const oldJobs = Array.from({ length: 7 }, (_, index) => ({
      ...
      status: "completed", terraProvider: "openrouter", terraModel: "z-ai/glm-5.3-flash", reasoningEffort: "high",
    ...
    const invoke = vi.fn()
      .mockResolvedValueOnce({ id: "ds-8", model: "z-ai/glm-5.3-flash", provider: "DeepSeek", choices: [{ message: { content: JSON.stringify(eightFields) } }] })
