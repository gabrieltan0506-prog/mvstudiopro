...
export const CREDIT_COSTS = {
  ...
  /** 图文知识卡片默认展示价（DeepSeek 档 30）；实际按 distillModel：DeepSeek V4.1 Flash 30/24、GLM 5.3 Flash 24/19；下架档按历史价 */
  ...
  /** 勾选后正式扩写文案 + 图文页结构：按条计费（2026-08-12 用户拍板 20 点/条；须与 shared/plans.ts 同步改） */
...
export const CREDIT_FEATURE_BREAKDOWN: readonly CreditFeatureBreakdownRow[] = [
  ...
  { product: "平台数据分析", subFeature: "初选扩写（按条计费，每条正式文案）", credits: CREDIT_COSTS.platformTopicExpand, note: "含 graphicNotePages 可发图文结构；失败条自动退款" },
  ...
  { product: "平台数据分析", subFeature: "小红书图文参考（2×4 八格）", credits: CREDIT_COSTS.platformXhsDualNote, note: "图文笔记 2×4 八格 72 cr/次（kind: xiaohongshu_dual_note）· 非分镜主表" },
  {
    ...
    note: `首次體驗 ${CREDIT_COSTS.decisionIntelligenceReportFirst} cr，之後每次 ${CREDIT_COSTS.decisionIntelligenceReport} cr；報告入庫沉澱`,
  },
