...
describe("冻结详细稿的真实平台worker接线", () => {
  it("只用队列账号、planId和mode，返回output.edition完整结构而非裸版本", async () => {
    ...
    expect(result).toEqual({ provider: "evolink", output: { success: true, edition, readingDonePages: 4, readingTotalPages: 4, readingPhase: "done" } });
    ...
    expect(patch).toHaveBeenCalledWith("edition-job", { readingDonePages: 4, readingTotalPages: 4, readingPhase: "writing" });
  ...
  it("详细编稿失败不返回空版本、不覆盖旧输出或调用阅读/收费清理边界", async () => {
    ...
    expect(patch.mock.calls.at(-1)).toEqual(["job", { readingDonePages: 2, readingTotalPages: 4, readingPhase: "writing" }]);
    expect(JSON.stringify(original)).toBe(snapshot);
    expect(unexpected).not.toHaveBeenCalled();
  });
