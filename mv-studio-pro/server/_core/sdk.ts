...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Step 1: Verify JWT signature (fast, in-memory)
    ...
    // Step 2: Check session exists in database (persistent validation)
    // This ensures logged-out sessions are rejected even if JWT is still valid
    if (sessionCookie) {
      const dbSession = await sessionDb.getSessionByToken(sessionCookie);
      if (!dbSession) {
        // Session not in DB — could be a legacy token created before persistence.
        // We allow it but log a warning. The token is still JWT-valid.
        console.warn(
          `[Auth] Token JWT-valid but not in sessions table (openId=${session.openId}). ` +
