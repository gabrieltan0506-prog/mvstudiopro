...
export const MANHUA_DIRECTOR_STRATEGY_FORMAT = "mv-manhua-director-strategy-v1" as const;
export const MANHUA_DIRECTOR_STRATEGY_VERSION = 1 as const;
...
export type ManhuaDirectorStrategyContract = {
  ...
  /** 前台与生产提示词可见的中性名称。 */
  labelZh: string;
  projections: Readonly<Record<ManhuaDirectorStrategyStage, ManhuaDirectorStrategyProjection>>;
};
...
> & {
  ...
  moduleIds: readonly ("spatial-previsualization" | "final-perceptual-allocation")[];
...
const STRATEGIES: readonly StrategySeed[] = [
  {
    ...
    labelZh: "信息因果推进",
    sourceProfileIds: ["architectural_contrast"],
    sourceClaimIds: ["CN-DM-02", "CN-DM-06", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["悬疑", "推理", "调查", "谜", "秘密", "时间", "多线", "谍战", "情报", "反转"],
    ...
    projections: {
      ...
      assets: {
        ...
        directivesZh: ["标明每条行动的空间起点、目标物和可见线索", "只把会改变判断的道具列为叙事锚点"],
        avoidZh: "装饰性资产不得冒充线索。",
      },
      storyboard: {
        objectiveZh: "每镜交出一个清楚的信息位置。",
        directivesZh: ["跨线前后保留目标、方向或时间锚", "切镜围绕决定性行动、关系变化与结果"],
      ...
      review: {
        ...
        directivesZh: ["删掉不改变目标、威胁、选择或结果的镜头", "确认关键揭示前后均有可辨证据"],
  ...
  },
  {
    ...
    labelZh: "情绪空间因果",
    sourceProfileIds: ["emotion_space_profile"],
    sourceClaimIds: ["HSCS-001", "HSCS-002", "HSCS-003", "HSCS-004", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["灾难", "末日", "战争", "史诗", "深海", "星际", "飞船", "巨物", "生存", "救援"],
    craftShotPrefixes: ["cam_07", "light_04", "emo_08"],
    projections: {
      story: {
        objectiveZh: "奇观先改变人物选择，再扩大尺度。",
        directivesZh: ["每个大场面先写人物要保护什么以及失败代价", "空间变化必须迫使人物改路线、关系或决定"],
        avoidZh: "禁止只有规模升级却没有人物后果。",
      },
      assets: {
        objectiveZh: "资产先服务眼线、接触、受力与路线。",
        directivesZh: ["列出可触碰的关键表面和接触点", "危险、尺度与不可控部分留给数字延展"],
        avoidZh: "不要为实体感建造无反馈、危险或昂贵的资产。",
      },
      storyboard: {
        objectiveZh: "先让表演与空间关系成立，再选择摄影。",
        directivesZh: ["建立方向、距离、眼线和接触结果", "关键动作按发起、路径、结果、反应组织覆盖"],
  ...
  },
  {
    ...
    labelZh: "角色视点动作",
    sourceProfileIds: ["kinetic_ensemble"],
    sourceClaimIds: ["KE-01", "KE-02", "KE-03", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["追逐", "赛车", "竞速", "竞技", "动作", "逃亡", "太空战", "群战", "闯关"],
    craftShotPrefixes: ["cam_03", "tr_04", "emo_06"],
    projections: {
      story: {
        objectiveZh: "动作规模由角色视点、目标和结果承受者决定。",
        directivesZh: ["每段动作写清谁发起、要到哪里、谁承担结果", "升级前先让人物目标或危险等级发生变化"],
      ...
      storyboard: {
        ...
        directivesZh: ["先建立方向轴，再用近景确认执行者与接触点", "只有目标、方向、危险或结果变化时才切"],
  ...
  },
  {
    ...
    labelZh: "观众感知揭示",
    sourceProfileIds: ["warm_wonder"],
    sourceClaimIds: ["AAD-001", "AAD-002", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["发现", "未知", "亲情", "家庭", "成长", "治愈", "重逢", "秘密身份", "初见"],
  ...
  },
  {
    ...
    labelZh: "具身世界叙事",
    sourceProfileIds: ["creature_gothic_awe"],
    sourceClaimIds: ["GDT-001", "GDT-002", "GDT-003", "GDT-005", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["怪物", "异类", "生物", "巨兽", "精灵", "妖", "非人", "奇幻", "异世界", "仪式"],
  ...
  },
  {
    ...
    labelZh: "关系驱动动作",
    sourceProfileIds: ["relational_action_profile"],
    sourceClaimIds: ["JWAR-001", "JWAR-002", "JWAR-003", "WSPV-001", "AB-D-01"],
    moduleIds: UNIVERSAL_MODULES,
    keywords: ["背叛", "兄弟", "忠诚", "牺牲", "复仇", "救人", "对决", "围攻", "枪战", "江湖"],
...
const BY_ID = new Map(STRATEGIES.map((entry) => [entry.strategyId, entry] as const));
...
): Pick<StrategySeed, "sourceProfileIds" | "sourceClaimIds" | "moduleIds"> | null {
...
export function parseManhuaDirectorStrategyContract(
  ...
  const input = raw as { format?: unknown; version?: unknown; strategyId?: unknown };
  if (input.format !== MANHUA_DIRECTOR_STRATEGY_FORMAT) return null;
  if (Number(input.version) !== MANHUA_DIRECTOR_STRATEGY_VERSION) return null;
...
export function resolveManhuaDirectorStrategyContract(input: {
  ...
  const text = `${String(input.topic || "")}\n${String(input.brief || "")}`.trim();
  for (const seed of STRATEGIES) {
    if (seed.keywords.some((keyword) => text.includes(keyword))) return toContract(seed);
  }
...
const STRATEGY_MARKER_RE = /【创作策略·v(\d+)·([a-z_]+)】/;
...
export function formatManhuaDirectorStrategyStage(
  ...
  return [
    `【创作策略·v${contract.version}·${contract.strategyId}】${contract.labelZh}`,
...
export function formatManhuaDirectorStrategyClipLine(
  ...
  return `【创作策略·v${contract.version}·${contract.strategyId}】${contract.labelZh}｜${projection.directivesZh.join("；")}｜边界：${projection.avoidZh}`;
}
...
export function readManhuaDirectorStrategyContract(
  ...
  if (!match || Number(match[1]) !== MANHUA_DIRECTOR_STRATEGY_VERSION) return null;
  return getManhuaDirectorStrategyContract(match[2]);
}
