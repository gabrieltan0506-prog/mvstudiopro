...
export type Image2TemplateGroupId = "portrait" | "lifestyle" | "scene" | "stylize";

export const IMAGE2_TEMPLATE_GROUP_ORDER: readonly {
  id: Image2TemplateGroupId;
  labelZh: string;
}[] = [
  { id: "portrait", labelZh: "人像处理" },
  { id: "lifestyle", labelZh: "生活笔记" },
  { id: "scene", labelZh: "场景造景" },
  { id: "stylize", labelZh: "风格玩法" },
] as const;

export type Image2PromptTemplate = {
  id: string;
  group: Image2TemplateGroupId;
  labelZh: string;
  blurbZh: string;
...
export function listImage2TemplatesByGroup(): Array<{
  group: (typeof IMAGE2_TEMPLATE_GROUP_ORDER)[number];
  items: Image2PromptTemplate[];
}> {
  return IMAGE2_TEMPLATE_GROUP_ORDER.map((group) => ({
    group,
    items: IMAGE2_PROMPT_TEMPLATES.filter((t) => t.group === group.id),
