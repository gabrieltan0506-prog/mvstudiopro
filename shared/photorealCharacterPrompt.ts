...
/**
 * 男主实拍同脸锚（用户已拍板：允许同脸复制 / 微调 / 融图）。
 * 第一张=库图构图底；其后实拍=人脸身份锚。
 */
export const PHOTOREAL_FACE_LOCK_BLEND_ZH = `【实拍同脸锚·允许融图】
- 实拍参考图是人脸身份锚：允许同脸复制；把该脸融进库图构图与服装，不要另生成一张无关脸。
- 须对齐：眼距、眉形、鼻型、唇形、颧骨与下颌底盘、耳形与发际线大轮廓；皮肤毛孔/血色跟实拍。
- 允许微调：表情、眼神、发型长短、服装、轻度年龄气质；骨相轮盘只允许在同一张脸上微调下颌宽度。
- 禁止磨皮瓷器皮；禁止换成名人或其他路人脸。`.trim();
