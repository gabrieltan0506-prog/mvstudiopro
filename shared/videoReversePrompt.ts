...
export const MANHUA_DRAMA_DEFAULT_PROMPTS: Record<ManhuaDramaStage, string> = {
  ...
  episode_beats:
    "把故事拆成 3–4 个可拍镜头节拍（单次合计严格 10 秒）：镜号｜景别｜运镜｜灯光情绪｜动作｜对白/旁白。每镜约 2–4 秒。这是导演灵感画布，不是死板清单。",
  video_reverse:
    "若已上传参考短片：反推八维编导分镜表 + Seedance 微动句。若无片：根据上游节拍补全八维分镜表（起承转合 + 观众必看点）。成稿去导演名。",
  key_art:
    "根据角色卡与编导分镜，生成竖屏关键静帧（角色锁定、电影感布光）。禁字硬锁：纯视觉、零可读文字；无字幕/对白气泡/水印/设定卡/姓名条/标题大字；对白只作表演依据不烧进画面；道具牌/屏幕仅模糊纹理。",
  seedance_clip:
    "有参考图时写：本镜事件动作 + 运镜 + 主体微动 + 氛围；单镜约 2–4 秒（整集合计约 10 秒）。禁止纯走路空镜；须演绎本镜关键事件。",
};
