...
export const MANHUA_DRAMA_DEFAULT_PROMPTS: Record<ManhuaDramaStage, string> = {
  story_brief:
    "用 5 行写出漫剧一集（单次 10 秒成片）：标题钩子、世界观一句、主角欲望、核心冲突、本集收束。禁止灌水。成稿去导演名/片名。",
  character_bible:
    "输出本集 1–3 个角色设定卡：外形锚点（发型/服装/标志物）、性格一句、对白语气、禁止崩坏点。成稿去真人名。",
  episode_beats:
    "把故事拆成 3–4 个可拍镜头节拍（单次合计严格 10 秒）。每镜一行必须写清：镜号｜景别·秒数｜机位｜肢体动作｜微表情｜台词（用「」）｜说话语气｜情绪弧。例：1. 近景4秒过肩：女主猛地抬头，眼眶发红泪未落，压哭腔沙哑问：「……？」委屈不信。台词只驱动口型气口，禁止写成烧字幕。禁止空镜走路。",
  video_reverse:
    "若已上传参考短片：反推八维编导分镜表 + 每镜表演字段（微表情/台词/语气/情绪）+ Seedance 微动句。若无片：根据上游节拍补全同上字段（起承转合 + 观众必看点）。成稿去导演名；台词禁止烧字。",
  key_art:
    "根据角色卡与编导分镜，生成竖屏关键静帧（角色锁定、电影感布光、近景微表情可读）。禁字硬锁：纯视觉、零可读文字；无字幕/对白气泡/水印/设定卡/姓名条/标题大字；对白只作表演依据不烧进画面；道具牌/屏幕仅模糊纹理。",
  seedance_clip:
    "有参考图时写：本镜事件动作 + 运镜 + 微表情过程 + 台词语气气口 + 氛围；单镜约 2–4 秒（整集合计约 10 秒）。禁止纯走路空镜；须演绎本镜关键事件与情绪落点；画面无新增字幕。",
};
