...
3. 有参考静帧时：写完整导戏（时间轴/对白/运镜/动作），禁止压成「运镜+微动+氛围」三件套空话。
4. 大幅度打斗拆成 2–3 秒阶段并写因果与环境反馈。
5. 八维每格尽量短词；不要写成说明书墙。
6. 中文为主（除非用户要求英文档）；景别/机位/运镜必须中文，禁止英文运镜术语；成片微动句也优先中文。`;
...
export const MANHUA_DRAMA_DEFAULT_PROMPTS: Record<ManhuaDramaStage, string> = {
  story_brief:
    "用 6–9 行写出漫剧一集（整集约 75–90 秒 / 约 5–6 段×15 秒）：①标题钩子 ②世界观一句 ③目标 ④阻力 ⑤代价 ⑥关键道具如何推动剧情 ⑦人物互动升级点（对白→肢体/对峙）⑧（可选）天气或氛围突变如何服务剧情 ⑨本集收束/片尾钩子。前三秒须有问题或异常。同场可多事件，须写清引爆与过渡。禁止灌水。成稿去导演名/片名。",
  character_bible:
    "输出本集 1–3 个角色设定卡（形象锁定用）：外形锚点（发型/服装/标志物）、性格一句、对白语气、与他人互动习惯（逼近/退让/护人）、禁止崩坏点。成稿去真人名。后续静帧与成片必须锁住同一形象，禁止换脸换装。",
  episode_beats:
    "把故事拆成约 5–6 段可拍节拍（整集约 75–90 秒；每段约 15 秒，段内约 3–4 镜静帧）。首段前 3 秒必须抛问题/异常/冲突。每段须有信息增量。对白密度：平均每段≥1 句，关键段 2–3 句，整集约 6–12 句。同场可多动作：写动作链 A→B→C；主角与其他人物须有互动轴线（对视/递接/推挡/逼近）。若晴转雨或对白转打斗：须有中介镜与引爆点。每镜一行：镜号｜景别｜运镜起落（必须中文，如低角仰拍/过肩跟拍/大特写推进，禁止 Low angle、push-in、OTS）｜动作链（优先中文）｜对手互动｜场景·天气状态｜道具入画｜微表情｜台词｜增量。台词只驱动口型。禁止空镜走路。",
  video_reverse:
    "若已上传参考短片：反推八维编导分镜表 + 表演字段 + 运镜起落（中文）+ 多拍动作链 + 人物互动 + 天气/氛围过渡 + 成片微动句（优先中文）。若无片：按上游节拍补全约 5–6 段（含对白↔冲突跳变、道具入画、目标阻力代价）。场次须含：画面/对白/镜头/动作链/互动/声音/转场/场景与天气渲染。成稿去导演名；台词禁止烧字；禁止英文运镜术语写入分镜说明。",
  key_art:
  ...
  seedance_clip:
    "15s 秒轴短指令：时间＋可见动作＋微表情＋说「对白」＋运镜；身份靠垫图@Image，勿堆规则墙/服化板。",
};
