...
export const MANHUA_DRAMA_DEFAULT_PROMPTS: Record<ManhuaDramaStage, string> = {
  ...
  episode_beats:
    "把故事拆成约 12 段可拍节拍（整集约 180 秒；每段约 15 秒，段内约 4 镜静帧）。首段前 3 秒必须抛问题/异常/冲突。每段须有信息增量。对白密度：平均每段≥1 句，关键段 2–3 句，整集约 12–20 句。同场可多动作：写动作链 A→B→C；主角与其他人物须有互动轴线（对视/递接/推挡/逼近）。若晴转雨或对白转打斗：须有中介镜与引爆点。每镜一行：镜号｜景别｜运镜起落（必须中文，如低角仰拍/过肩跟拍/大特写推进，禁止 Low angle、push-in、OTS）｜动作链（优先中文）｜对手互动｜场景·天气状态｜道具入画｜微表情｜台词｜增量。台词只驱动口型。禁止空镜走路。",
  video_reverse:
    "若已上传参考短片：反推八维编导分镜表 + 表演字段 + 运镜起落（中文）+ 多拍动作链 + 人物互动 + 天气/氛围过渡 + 成片微动句（优先中文）。若无片：按上游节拍补全约 12 段（含对白↔冲突跳变、道具入画、目标阻力代价）。场次须含：画面/对白/镜头/动作链/互动/声音/转场/场景与天气渲染。成稿去导演名；台词禁止烧字；禁止英文运镜术语写入分镜说明。",
  key_art:
  ...
  seedance_clip:
    "有参考图时写完整视频导戏单（一轮约 15 秒）：每镜含时间轴秒位｜切镜｜运镜起落｜场景｜动作/微表情｜对白台词与语气（引擎同轮有声+口型）。同场锁脸服场。若对白转打斗或晴转雨，中间用可见转折。先锁形象再动；禁止哑巴空镜；禁止压成运镜+微动+氛围三件套；画面无字幕；运镜中文。",
};
