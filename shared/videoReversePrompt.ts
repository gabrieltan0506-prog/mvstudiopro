...
export const MANHUA_DRAMA_DEFAULT_PROMPTS: Record<ManhuaDramaStage, string> = {
  story_brief:
    "用 6 行写出漫剧一集（单次 10 秒成片）：①标题钩子 ②世界观一句 ③目标（主角必须拿到什么）④阻力（谁/什么在挡）⑤代价（失败失去什么）⑥本集收束/片尾钩子。前三秒须有问题或异常。禁止灌水。成稿去导演名/片名。",
  character_bible:
    "输出本集 1–3 个角色设定卡（形象锁定用）：外形锚点（发型/服装/标志物）、性格一句、对白语气、禁止崩坏点。成稿去真人名。后续静帧与成片必须锁住同一形象，禁止换脸换装。",
  episode_beats:
    "把故事拆成 3–4 个可拍镜头节拍（单次合计严格 10 秒）。首镜≤3 秒必须抛问题/异常/冲突。每镜须有信息增量（新信息/冲突/关系/危机/转折/结果之一）。每镜一行：镜号｜景别·秒数｜运镜（低角仰拍/过肩/特写/细节特写/跟随/手持等择一）｜肢体动作｜微表情｜台词（「」）｜说话语气｜情绪弧｜本镜信息增量。例：1. 近景3秒过肩：女主猛地抬头，眼眶发红泪未落，压哭腔问：「……？」委屈不信｜增量=异常开场。台词只驱动口型气口，禁止烧字幕。禁止空镜走路。少写心理，多写可见动作与道具信息。",
  video_reverse:
    "若已上传参考短片：反推八维编导分镜表 + 每镜表演字段（微表情/台词/语气/情绪）+ 运镜词 + 成片微动句。若无片：根据上游节拍补全同上（起承转合 + 观众必看点 + 目标阻力代价）。场次可带：画面/对白/镜头/声音/转场。成稿去导演名；台词禁止烧字。",
  key_art:
    "根据角色卡与编导分镜，生成竖屏关键静帧（角色锁定、电影感布光、近景微表情可读）。形象须与角色卡一致。禁字硬锁：纯视觉、零可读文字；无字幕/对白气泡/水印/设定卡/姓名条/标题大字；对白只作表演依据不烧进画面；道具牌/屏幕仅模糊纹理。",
  seedance_clip:
    "有参考图时写：本镜事件动作 + 单一主运镜 + 微表情过程 + 台词语气气口 + 氛围；单镜约 2–4 秒（整集合计约 10 秒）。先锁形象再动；单镜勿堆多种运镜。禁止纯走路空镜；须演绎本镜关键事件与情绪落点；画面无新增字幕。",
};
