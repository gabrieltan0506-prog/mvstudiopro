...
/**
 * 知识卡出图像素质：一律 high（4K）。
 * 旧规「超过 6 页整套降 2K」已按 2026-09-08 用户拍板取消，页数不影响清晰度。
 */
export function knowledgeCardImageQuality(_pageTotal?: number): "high" {
  return "high";
}
...
export type KnowledgeCardDistillTradeoff = {
  /** 直接出图：页数、总积分；is4k 恒为 true（页数不再影响清晰度） */
...
function packByCap(full: string, h1: string, cap: number): string[] {
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    // 只在「标题+下一单元」装得进一页时才粘连；下一单元本身接近整页时不粘，避免造出只剩标题的空页
    const gluedLen = i + 1 < units.length ? unit.length + 2 + units[i + 1]!.length : unit.length;
    const glued = isHeading && gluedLen <= room ? gluedLen : unit.length;
...
const KNOWLEDGE_CARD_INTERNAL_DIRECTIVE_HEADS = [
  ...
  "【DeepResearch Pro",
  "【3×4 跨段视觉真源】",
  // 图文可视化版式块（composeInfographicScriptContext）：用户 2026-08-05 选「左右对半对比」后，
  // 第 1 页整页印成了模板说明书（标题即 labelZh，六个 SECTION 即版式元结构）
  "【图文可视化模板",
  "【信息可视化·版式元结构",
  "【内容锁定·强制】",
  "【用户正文·唯一内容来源】",
...
const KNOWLEDGE_CARD_INTERNAL_JARGON_STRONG = [
  ...
  "每格留「下一拍」",
  "创意母题（改写进用户场景）",
  // 版式模板术语：模型会把版式块改写成正常 `## 小节`，只按 `【…】` 块头抓不到
  "图文可视化模板",
  "信息可视化·版式元结构",
  "仅版式",
  "LAYOUT ONLY",
