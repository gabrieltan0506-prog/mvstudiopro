 ...
 * - 高级版（full）：内容完整，不舍章节。按约每 900 字 1 节铺开，且不少于精简版的两倍，上限 120 节
 *   （0908 探针：按 1,400 字/节 + 统稿压缩后只剩 10 页，用户判「比纯文字还少」，改为不压缩）。
 *
...
export function suggestKnowledgeCardMinSections(
  ...
  return Math.min(120, Math.max(concise * 2, Math.ceil(n / 900)));
}
