...
2. **第 1 页（P1）必须是目录/议程（viz=steps 或 hub）**，逐条列出用户已确认的 confirmedThemes（themeId + themeTitle），后续页按主题展开，每页须带 themeId/themeTitle 挂靠对应主题。
...
function formatConfirmedThemes(themes?: HtmlPptTheme[]): string {
  ...
  return [
    ...
    ...themes.map((t, i) => `${i + 1}. [${t.id}] ${t.title}`),
  ].join("\n");
}
...
export function buildHtmlPptOutlineUserPrompt(input: HtmlPptOutlineLlmInput): string {
  ...
  return [
    ...
    themesBlock
      ? "P1 必须是目录/议程（steps 或 hub），列出全部 confirmedThemes；每页带 themeId/themeTitle。"
      : "",
