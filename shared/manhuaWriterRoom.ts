...
export function buildManhuaWriterExpandPrompt(opts: {
  ...
  return [
    ...
    "3. 单集目标约 180 秒 = 正好 12 段 × 约 15 秒；对白/场面须撑满密度，禁止寒暄灌水与段间复制粘贴。",
    ...
    "10. 每一集「本集剧情」之后必须输出完整「十二段可拍表」（段01–段12），字段见下文模板；缺段或缺字段视为未完成。",
    "",
  ...
  return [
    ...
    ep
      ? [
          ...
          segmentBlock,
          `片尾钩子：${ep.endHook}`,
          "本轮制作先兑现这一集（约 12 段×15 秒）；钩子留给下一集，勿在本集拍穿。",
          "人物/道具用系列表锁定；本集主场景见资产真源，可切场景池其他地点但须有过渡。",
          "节拍/静帧/成片须对齐十二段可拍表字段，禁止把一集压成几条短视频口播。",
        ].join("\n")
      : "",
  ]
    .filter(Boolean)
