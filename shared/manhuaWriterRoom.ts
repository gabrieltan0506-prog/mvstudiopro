...
export function buildManhuaWriterExpandPrompt(opts: {
  ...
  return [
    ...
    "3. 单集目标约 150–180 秒 = 10–12 段 × 约 15 秒（推荐 12 段）；对白/场面须撑满密度，禁止寒暄灌水与段间复制粘贴。对白一律用直角引号「」包裹（勿只用弯引号“”）。每段约15秒须至少 3 句「」对白（推荐 3–4 句），并写「表演」栏（表情/肢体/情绪起伏）；禁止两句口号撑满一段。",
    ...
    "10. 每一集「本集剧情」之后必须输出完整「十至十二段可拍表」（至少段01–段10，推荐到段12），字段见下文模板（含对白+表演）；缺段、缺字段、对白不足 3 句「」、表演过薄视为未完成。人物姓名/场景名必须与人物表·场景表一致，禁止另造皇宫大殿等未立场景。",
