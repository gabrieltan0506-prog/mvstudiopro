...
export function buildManhuaWriterExpandPrompt(opts: {
  ...
  return [
    ...
    `3. 成片档位已定：${layout.labelZh} → 单集目标约 ${targetSec} 秒 = ${segMin}–${segMax} 段 × 约 ${segDur} 秒（推荐 ${segDefault} 段）。对白/场面须撑满密度，禁止寒暄灌水与段间复制粘贴。对白一律用直角引号「」包裹（勿只用弯引号“”）。每段约 ${segDur} 秒按自然语速、动作和停顿安排对白，不规定句数；写「表演」栏（表情/肢体/情绪起伏），无对白段明确写无对白及行动。`,
    ...
    `9. 每一集「本集剧情」之后必须输出完整「${planTitle}」（正好 ${segDefault} 段：段01–段${String(segDefault).padStart(2, "0")}），字段见下文模板（含对白+表演）；缺段、缺字段、因果断裂或缺少可拍行动视为未完成。人物姓名/场景名必须与人物表·场景表一致，禁止另造皇宫大殿等未立场景。`,
  ...
  return [
    ...
    ep
      ? [
          ...
          "节拍、静帧和成片须对齐实际分段表及原镜编号；按本段真实镜头锁定关键静帧，不补假镜、不压缩原稿凑固定段数。",
        ].join("\n")
      : "",
  ]
    .filter(Boolean)
