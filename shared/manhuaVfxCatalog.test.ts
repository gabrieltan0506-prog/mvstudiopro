...
describe("分类特效文字配方",()=>{
 it("12类至少5条，稳定编号与名称唯一，每条有完整机制与边界",()=>{
  expect(MANHUA_VFX_CATEGORIES).toHaveLength(12);expect(MANHUA_VFX_CATALOG).toHaveLength(66);
  expect(new Set(MANHUA_VFX_CATALOG.map(p=>p.id)).size).toBe(66);expect(new Set(MANHUA_VFX_CATALOG.map(p=>p.nameZh)).size).toBe(66);
 ...
 it("编译非空并逐字保留自定义意图，不注入电影或角色名，不冒充白模支持",()=>{
  ...
  for(const p of MANHUA_VFX_CATALOG){const plain=formatManhuaVfxDirection(p.id);expect(plain).toContain(p.descriptionZh);expect(plain).not.toMatch(/速度与激情|复仇者联盟|美国队长|星际迷航|黑客帝国|Matrix|白模已支持/);expect(formatManhuaVfxDirection(p.id,custom)).toContain(`\n${custom}`);}
