 ...
 * - op=vertexTranslate (Vertex IAM 纯文本翻译；location=us-central1、模型 gemini-3.1-flash-live-preview-latest，供 TestLab 验证)
...
export default async function handler(req:VercelRequest,res:VercelResponse){
  try{
    ...
    // ---------------- Vertex：TestLab 翻译（Flash live preview latest、us-central1） ----------------
    if (op === "vertexTranslate") {
      ...
      const model = "gemini-3.1-flash-live-preview-latest";
      const base = baseUrlFor(location);
      const url = `${base}/v1/projects/${projectId}/locations/${location}/publishers/google/models/${model}:generateContent`;
