...
/** vertexTranslate：先 global；仅 HTTP 400（请求体/参数问题）不重试另一区域，其余错误再试 `us-central1`。 */
function vertexTranslateShouldTryRegionalFallback(status: number): boolean {
  return status !== 400;
}
  ...
  try{
    ...
    if (op === "vertexTranslate") {
      ...
      for (let i = 0; i < locations.length; i++) {
        ...
        if (r.ok) break;
        if (i === 0 && vertexTranslateShouldTryRegionalFallback(r.status)) {
      ...
      return res.status(r.ok ? 200 : 502).json({
        ...
        ...(primaryMeta && usedLocation === locations[1]
